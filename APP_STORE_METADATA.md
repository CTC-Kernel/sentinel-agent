# 📱 Métadonnées App Store Connect - Sentinel GRC

## Informations de Base

**Nom de l'application**: Sentinel GRC

**Sous-titre**: Gouvernance, Risques & Conformité ISO 27001

**Bundle ID**: com.sentinel.grc

**SKU**: sentinel-grc-2024

**Langue principale**: Français

**Catégorie principale**: Business

**Catégorie secondaire**: Productivity

---

## Description (4000 caractères max)

Sentinel GRC est la plateforme professionnelle de gestion de la sécurité des systèmes d'information (SSI) conforme aux normes ISO 27001 et ISO 27005.

FONCTIONNALITÉS PRINCIPALES:

✓ Gestion des Actifs
- Inventaire complet des actifs IT
- Classification et évaluation de criticité
- Suivi du cycle de vie

✓ Gestion des Risques
- Identification des menaces et vulnérabilités
- Évaluation selon ISO 27005
- Plans de traitement automatisés
- Tableaux de bord dynamiques

✓ Audits de Conformité
- Grilles ISO 27001 intégrées
- Collecte de preuves
- Génération automatique de rapports PDF
- Suivi des écarts et recommandations

✓ Gestion de Projets SSI
- Planification et suivi
- Jalons et échéances
- Lien avec contrôles ISO

✓ Conformité ISO 27001
- Tableau de bord par annexe A
- Statement of Applicability (SoA)
- Suivi de mise en œuvre

✓ Gestion Documentaire
- Stockage sécurisé
- Versionning
- Validation et publication

DESTINÉ AUX:
- RSSI (Responsables de la Sécurité des Systèmes d'Information)
- Équipes GRC (Gouvernance, Risques, Conformité)
- Consultants en cybersécurité
- Auditeurs internes/externes
- DPO (Data Protection Officers)

SÉCURITÉ & CONFIDENTIALITÉ:
- Chiffrement de bout en bout
- Authentification multi-facteurs
- Conformité RGPD
- Hébergement sécurisé Firebase
- Aucune donnée partagée avec des tiers

SUPPORT:
- Documentation complète
- Support technique dédié
- Mises à jour régulières

---

## Mots-clés (100 caractères max, séparés par virgules)

ISO 27001, GRC, cybersécurité, conformité, audit, risques, RSSI, sécurité, SMSI, ISO 27005

---

## URLs

**URL de support**: https://sentinel-grc.com/support

**URL marketing**: https://sentinel-grc.com

**URL Politique de confidentialité**: https://sentinel-grc.com/privacy

---

## Notes de Version (What's New)

Version 2.0.0

Nouvelle version complète de Sentinel GRC avec:

• Interface utilisateur moderne et intuitive
• Tableaux de bord interactifs
• Génération automatique de rapports PDF
• Gestion complète des risques ISO 27005
• Audits de conformité ISO 27001
• Gestion documentaire intégrée
• Performance optimisée
• Sécurité renforcée

---

## Informations de Révision

**Prénom**: [Votre prénom]
**Nom**: [Votre nom]
**Téléphone**: [Votre téléphone]
**Email**: support@sentinel-grc.com

**Notes pour l'équipe de révision**:

Sentinel GRC est une plateforme professionnelle de gestion de la sécurité des systèmes d'information conforme ISO 27001.

Fonctionnalités principales:
- Gestion des actifs IT
- Évaluation des risques (ISO 27005)
- Audits de conformité
- Génération de rapports PDF
- Tableaux de bord de conformité

Compte de test fourni ci-dessous.

**Compte de démo**:
- Email: demo@sentinel-grc.com
- Password: SentinelDemo2024!

L'application est pré-remplie avec des données de démonstration pour faciliter la révision.

---

## Copyright

© 2024 Sentinel GRC. Tous droits réservés.

---

## Âge Minimum

4+ (Aucun contenu sensible)

---

## Prix

Gratuit avec achats intégrés (ou définir un prix fixe)

---

## Disponibilité

Tous les pays

---

## Export Compliance

**Uses Encryption**: YES
**Exempt from regulations**: YES (HTTPS standard uniquement)
**ITSAppUsesNonExemptEncryption**: false
