...
export const useTeamManagement = (enabled = true) => {
        ...
        const unsubInvites = onSnapshot(query(collection(db, 'invitations'), where('organizationId', '==', orgId), limit(50)), (snap) => {
            const pending = snap.docs.map(d => {
                ...
                return {
                    uid: d.id,
                    email: inv.email,
                    displayName: t('team.pendingInvite', { defaultValue: 'Invited (Pending)' }),
                    role: inv.role,
    ...
    const inviteUser = useCallback(async (data: UserFormData, silent = false) => {
        ...
        try {
            ...
            await sendEmail(user, {
                to: data.email,
                subject: t('team.invitationSubject', { org: user.organizationName || 'Sentinel GRC', defaultValue: 'Invitation to join {{org}}' }),
                type: 'INVITATION',
            ...
            if (!silent) addToast(t('team.toast.invitationSent', { defaultValue: "Invitation sent" }), "success");
            return true;
        } catch (error) {
    ...
    const updateUser = async (uid: string, data: Partial<UserFormData>, isPending: boolean) => {
        ...
        try {
            ...
            addToast(t('team.toast.updated', { defaultValue: "Updated" }), "success");
            return true;
        } catch (error) {
    ...
    const deleteUser = async (u: UserProfile) => {
        ...
        try {
            ...
            addToast(t('team.toast.deleted', { defaultValue: "Deleted" }), "success");
            return true;
        } catch (error) {
    ...
    const approveRequest = async (req: JoinRequest) => {
        ...
        try {
            ...
            addToast(t('team.toast.approved', { defaultValue: "Approved: {{name}}", name: req.displayName }), "success");
            return true;
        } catch (e) {
    ...
    const rejectRequest = async (req: JoinRequest) => {
        ...
        try {
            ...
            addToast(t('team.toast.rejected', { defaultValue: "Rejected" }), "info");
            return true;
        } catch (e) {
    ...
    const importUsers = useCallback(async (csvData: string) => {
        ...
        try {
            ...
            if (rows.length === 0) return addToast(t('common.toast.emptyFile', { defaultValue: "Empty file" }), "error");
            ...
            addToast(t('team.toast.usersImported', { defaultValue: "{{count}} users imported", count: done }), "success");
        } finally {
            setLoading(false);
        }
