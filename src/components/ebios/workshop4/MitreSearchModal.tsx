...
export const MitreSearchModal: React.FC<MitreSearchModalProps> = ({
 ...
 className="w-full pl-12 pr-4 py-3 rounded-3xl border border-border/40 bg-card text-foreground"
 />
 </div>
 ...
 <div className="w-1/3 border-r border-border/40/50 overflow-y-auto pr-4">
 {/* Suggestions Section */}
 ...
 {searchResults.length === 0 ? (
  ...
  >
  <code className="text-xs bg-muted px-1.5 py-0.5 rounded text-foreground text-muted-foreground">
  {technique.id}
  </code>
  <span className="text-foreground truncate">
  {technique.name[locale]}
 ...
 <div className="space-y-1">
  ...
  <span className="w-5 h-5 rounded bg-muted text-xs flex items-center justify-center font-medium">
  {tactic.order}
  </span>
 ...
 <div className="space-y-3">
  ...
  <div
  key={technique.id || 'unknown'}
  className="rounded-3xl border border-border/40 overflow-hidden"
  >
  {/* Technique Header */}
  <button
  onClick={() => handleSelectTechnique(technique)}
 ...
 {/* Footer */}
 <div className="mt-4 pt-4 border-t border-border/40/50 flex justify-between items-center">
