...
const WORKSHOP_ACTIONS: Record<EbiosWorkshopNumber, { action: AIAction; labelKey: string; defaultLabel: string; icon: React.ReactNode }[]> = {
 1: [
 { action: 'suggest_feared_events', labelKey: 'ebios.ai.suggestFearedEvents', defaultLabel: 'Suggérer événements redoutés', icon: <Target className="h-3.5 w-3.5" /> }
 ],
 2: [
 { action: 'analyze_risk_sources', labelKey: 'ebios.ai.analyzeRiskSources', defaultLabel: 'Analyser sources de risque', icon: <Crosshair className="h-3.5 w-3.5" /> }
 ],
 3: [
 { action: 'generate_strategic_scenarios', labelKey: 'ebios.ai.generateStrategicScenarios', defaultLabel: 'Générer scénarios stratégiques', icon: <GitBranch className="h-3.5 w-3.5" /> }
 ],
 4: [
 { action: 'generate_operational_scenarios', labelKey: 'ebios.ai.generateOperationalScenarios', defaultLabel: 'Générer scénarios opérationnels', icon: <Shield className="h-3.5 w-3.5" /> }
 ],
 5: [
 { action: 'suggest_treatment', labelKey: 'ebios.ai.suggestTreatment', defaultLabel: 'Suggérer traitement', icon: <CheckSquare className="h-3.5 w-3.5" /> }
 ]
};
...
export const EbiosAIAssistant: React.FC<EbiosAIAssistantProps> = ({
 ...
 setError(t('ebios.ai.error', { defaultValue: "Une erreur est survenue lors de l'analyse IA." }));
 } finally {
 setLoading(false);
 }
 };
 ...
 let label = t('common.apply', { defaultValue: 'Appliquer' });

 switch (activeAction) {
 case 'suggest_feared_events':
 if (response.fearedEvents?.length) handler = handleApplyFearedEvents;
 label = t('ebios.ai.applyFearedEvents', { defaultValue: 'Ajouter les événements' });
 break;
 case 'generate_strategic_scenarios':
 if (response.strategicScenarios?.length) handler = handleApplyStrategicScenarios;
 label = t('ebios.ai.applyStrategic', { defaultValue: 'Ajouter les scénarios' });
 break;
 case 'generate_operational_scenarios':
 if (response.operationalScenarios?.length) handler = handleApplyOperationalScenarios;
 label = t('ebios.ai.applyOperational', { defaultValue: 'Ajouter les scénarios' });
 break;
 case 'suggest_treatment':
 if (response.treatmentSuggestions?.length) handler = handleApplyTreatment;
 label = t('ebios.ai.applyTreatment', { defaultValue: 'Ajouter au plan de traitement' });
 break;
 }
 ...
 return (
 <div className="bg-white/50/40 rounded-2xl p-4 border border-border/40 shadow-sm animate-fade-in backdrop-blur-md">
 <div className="flex justify-between items-start mb-3">
 <h4 className="text-xs font-bold uppercase tracking-wider text-primary flex items-center">
 <Bot className="h-3.5 w-3.5 mr-1.5" />
 {t('ebios.ai.response', { defaultValue: 'Suggestions IA' })}
 </h4>
 <button
 ...
 <div key={i || 'unknown'} className="p-2 bg-muted rounded-lg">
  ...
  <span className="text-xs text-primary">{e.impactType}</span>
 </div>
 ))}
 </div>
 )}
 ...
 <div className="space-y-2">
 <div>
 <span className="font-medium text-success">{t('ebios.ai.relevant', { defaultValue: 'Pertinentes' })}:</span>
 ...
 <div>
  <span className="font-medium">{t('ebios.ai.recommendations', { defaultValue: 'Recommandations' })}:</span>
  <ul className="list-disc pl-4 text-xs mt-1">
 ...
 <div key={i || 'unknown'} className="p-2 bg-muted rounded-lg">
  ...
  <p className="text-xs text-primary dark:text-primary/50 mt-1">
 ...
 <div key={i || 'unknown'} className="p-2 bg-muted rounded-lg">
  ...
  <span className="font-mono bg-muted px-1 rounded">{j + 1}</span>
 ...
 <div key={i || 'unknown'} className="p-2 bg-muted rounded-lg">
  ...
  <div className="mt-2">
  <span className="text-xs font-medium">{t('ebios.ai.controls', { defaultValue: 'Contrôles suggérés' })}:</span>
 ...
 return (
 <div className="bg-gradient-to-br from-primary/10/50 to-violet-50/50 dark:from-primary/10 dark:to-violet-900/10 rounded-3xl p-6 border border-border/40 shadow-sm">
 {/* Header */}
 ...
 {t('ebios.ai.title', { defaultValue: 'Assistant IA EBIOS RM' })}
 </h3>
 <p className="text-xs text-muted-foreground">
 {t('ebios.ai.subtitle', { defaultValue: 'Atelier' })} {currentWorkshop}
 </p>
 </div>
 </div>
 ...
 {actions.map(({ action, labelKey, defaultLabel, icon }) => (
 <button
 key={action || 'unknown'}
