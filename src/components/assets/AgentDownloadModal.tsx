...
export const AgentDownloadModal: React.FC<AgentDownloadModalProps> = ({
 ...
 <div className="fixed inset-0 z-decorator w-screen overflow-y-auto">
  ...
  <div className="relative p-6 px-8 pt-10 text-center">
   ...
   {t('assets.agentDownload.title', { defaultValue: 'Sécuriser cet actif ?' })}
   </Dialog.Title>
   <p className="text-sm text-muted-foreground leading-relaxed max-w-xs mx-auto mb-6">
   {t('assets.agentDownload.description', { defaultValue: "L'actif {{assetName}} est éligible à la protection Sentinel Agent.", assetName: assetName || t('assets.agentDownload.created', { defaultValue: 'créé' }) })}
   </p>

   <div className="bg-muted/30 rounded-2xl p-4 mb-8 text-left border border-border/50">
   <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">{t('assets.agentDownload.featuresIncluded', { defaultValue: 'Fonctionnalités incluses' })}</h4>
   ...
   <li className="flex items-center gap-2.5 text-sm text-foreground">
    ...
    {t('assets.agentDownload.feature1', { defaultValue: 'Collecte de logs en temps réel' })}
   </li>
   <li className="flex items-center gap-2.5 text-sm text-foreground">
    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-success/10 text-success ring-1 ring-success/20">
    <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
    </span>
    {t('assets.agentDownload.feature2', { defaultValue: 'Remontée des vulnérabilités (CVE)' })}
   </li>
   <li className="flex items-center gap-2.5 text-sm text-foreground">
    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-success/10 text-success ring-1 ring-success/20">
    <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
    </span>
    {t('assets.agentDownload.feature3', { defaultValue: 'Configuration automatique' })}
   </li>
   </ul>
   </div>
   </motion.div>
   ...
   {t('assets.agentDownload.downloadAgent', { defaultValue: "Télécharger l'Agent" })}
   </Button>
   <Button
   type="button"
   variant="ghost"
   ...
   >
   {t('assets.agentDownload.later', { defaultValue: 'Plus tard' })}
   </Button>
   </div>
  </div>
  </Dialog.Panel>
