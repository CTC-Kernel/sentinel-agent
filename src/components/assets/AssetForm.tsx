...
export const AssetForm: React.FC<AssetFormProps> = ({
 ...
 <fieldset disabled={readOnly} className="space-y-8 disabled:text-muted-foreground">
 <div className="glass-premium p-6 rounded-3xl shadow-sm border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.mainInfo', { defaultValue: 'Informations Principales' })}</h3>
  ...
  <Controller
   ...
   render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.assetName', { defaultValue: "Nom de l'actif" })}
   id="name"
   autoComplete="off"
   {...field}
  ...
  <button
   ...
   title={t('assets.form.suggestName', { defaultValue: 'Suggérer un nom' })}
  >
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.type', { defaultValue: 'Type' })}
   options={ASSET_TYPES.map(assetType => ({ value: assetType, label: assetType }))}
   value={field.value}
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.owner', { defaultValue: 'Propriétaire' })}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.location', { defaultValue: 'Localisation' })}
   id="location"
   autoComplete="off"
   {...field}
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.supplierMaintainer', { defaultValue: 'Fournisseur / Mainteneur' })}
   options={suppliers.map(s => ({ value: s.id, label: s.name, subLabel: s.category }))}
   value={field.value || ''}
   onChange={field.onChange}
   placeholder={t('assets.form.selectSupplier', { defaultValue: 'Sélectionner un fournisseur...' })}
 ...
 {watch('type') === 'Matériel' && (
  ...
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.hardwareDetails', { defaultValue: 'Détails Matériel' })}</h3>
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.ipAddress', { defaultValue: 'Adresse IP' })}
   id="ipAddress"
   autoComplete="off"
   {...field}
 ...
 {watch('type') === 'Logiciel' && (
  <div className="glass-premium p-6 rounded-3xl shadow-sm animate-fade-in border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.softwareDetails', { defaultValue: 'Détails Logiciel' })}</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  <Controller
  name="version"
  control={control}
  ...
  <Controller
   ...
   render={({ field }) => (
   <DatePicker
   label={t('assets.form.licenseExpiry', { defaultValue: 'Expiration Licence' })}
   value={field.value}
   onChange={field.onChange}
 ...
 {watch('type') === 'Humain' && (
  <div className="glass-premium p-6 rounded-3xl shadow-sm animate-fade-in border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.employeeDetails', { defaultValue: 'Détails Collaborateur' })}</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  <Controller
  name="email"
  control={control}
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.professionalEmail', { defaultValue: 'Email Professionnel' })}
   id="email"
   autoComplete="email"
   {...field}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.functionRole', { defaultValue: 'Fonction / Rôle' })}
   id="role"
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.department', { defaultValue: 'Département' })}
   id="department"
 ...
 {watch('type') === 'Service' && (
  <div className="glass-premium p-6 rounded-3xl shadow-sm animate-fade-in border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.serviceDetails', { defaultValue: 'Détails Service / SaaS' })}</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  <Controller
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.providerUrl', { defaultValue: 'URL du Fournisseur' })}
   id="providerUrl"
   autoComplete="url"
   {...field}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.slaLevel', { defaultValue: 'Niveau SLA (ex: 99.9%)' })}
   id="sla"
   autoComplete="off"
   {...field}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.supportContact', { defaultValue: 'Contact Support' })}
   id="supportContact"
   autoComplete="tel"
   {...field}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.hostingLocation', { defaultValue: "Lieu d'hébergement" })}
   id="hostingLocation"
   autoComplete="off"
   {...field}
 ...
 {watch('type') === 'Données' && (
  <div className="glass-premium p-6 rounded-3xl shadow-sm animate-fade-in border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.dataDetails', { defaultValue: 'Détails Données' })}</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  <Controller
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.format', { defaultValue: 'Format' })}
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.dataCategory', { defaultValue: 'Catégorie de Données' })}
  ...
  render={({ field }) => (
   <FloatingLabelInput
   label={t('assets.form.retentionPeriod', { defaultValue: 'Durée de rétention (ex: 5 ans)' })}
   id="retentionPeriod"
   autoComplete="off"
   {...field}
  ...
  <label htmlFor="isEncrypted" className="flex items-center space-x-3 cursor-pointer group">
   ...
   type="checkbox"
   id="isEncrypted"
   className="form-checkbox h-5 w-5 text-primary rounded border-border/40 focus-visible:ring-primary transition-all duration-200"
   />
   <span className="text-sm font-medium text-foreground group-hover:text-primary dark:group-hover:text-primary/70 transition-colors">{t('assets.form.encryptedData', { defaultValue: 'Données Chiffrées (At rest / Transit)' })}</span>
  </label>
  <label htmlFor="hasWorm" className="flex items-center space-x-3 cursor-pointer group">
   ...
   className="form-checkbox h-5 w-5 text-primary rounded border-border/40 focus-visible:ring-primary transition-all duration-200"
   />
   <span className="text-sm font-medium text-foreground group-hover:text-primary dark:group-hover:text-primary/70 transition-colors">{t('assets.form.immutableStorage', { defaultValue: 'Stockage Immuable (WORM)' })}</span>
  </label>
  </div>
  </div>
  </div>
 ...
 <div className="glass-premium p-6 rounded-3xl shadow-sm border border-border/40">
  ...
  <div className="flex items-center"><AlertTriangle className="h-4 w-4 mr-2" /> {t('assets.form.dicClassification', { defaultValue: 'Classification DIC' })}</div>
  <button
  type="button"
  onClick={handleAutoClassify}
  className="text-xs normal-case font-bold text-primary hover:text-foreground hover:bg-primary/90 flex items-center bg-card px-3 py-1.5 rounded-lg transition-all border border-primary/30 dark:border-primary/90 shadow-sm hover:shadow-md"
  >
  <Sparkles className="h-3 w-3 mr-1.5" /> {t('assets.form.autoClassify', { defaultValue: 'Auto-Classifier (Règles)' })}
  </button>
  </h3>
 ...
 <div className="glass-premium p-6 rounded-3xl shadow-sm border border-border/40">
  ...
  <ShieldCheck className="h-4 w-4 mr-2" /> {t('assets.form.complianceScope', { defaultValue: 'Périmètre de Conformité (Scope)' })}
  </h3>
  ...
  className={`cursor-pointer px-4 py-2 rounded-3xl border transition-all ${(watch('scope') || []).includes(scope)
   ? 'bg-primary/10/50 dark:bg-primary/20 border-primary/50 text-primary dark:text-primary/50 font-bold shadow-sm'
   : 'border-border/40 hover:bg-muted/50 dark:hover:bg-muted/50 text-foreground'
 ...
 <div className="glass-premium p-6 rounded-3xl shadow-sm border border-border/40">
  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">{t('assets.form.lifecycle', { defaultValue: 'Cycle de Vie' })}</h3>
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  <div>
  <Controller
  ...
  render={({ field }) => (
   <CustomSelect
   label={t('assets.form.status', { defaultValue: 'Statut' })}
  ...
  render={({ field }) => (
   <DatePicker
   label={t('assets.form.purchaseDate', { defaultValue: "Date d'achat" })}
   value={field.value}
   onChange={field.onChange}
  ...
  render={({ field }) => (
   <DatePicker
   label={t('assets.form.warrantyEnd', { defaultValue: 'Fin de garantie' })}
   value={field.value}
   onChange={field.onChange}
  ...
  render={({ field }) => (
   ...
   id="purchasePrice"
   autoComplete="off"
   label={t('assets.form.purchasePrice', { defaultValue: "Prix d'achat (€)" })}
   value={field.value}
 ...
 <div className="flex justify-end gap-3 pt-6 border-t border-border/40">
  ...
  disabled={isLoading}
  >
  {t('assets.form.cancel', { defaultValue: 'Annuler' })}
  </Button>
  <Button
  type="submit"
  isLoading={isLoading}
  variant="premium"
  >
  {isEditing ? t('assets.form.update', { defaultValue: 'Mettre à jour' }) : t('assets.form.createAsset', { defaultValue: "Créer l'actif" })}
  </Button>
 </div>
 )}
 </form>
