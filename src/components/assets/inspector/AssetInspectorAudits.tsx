                    ...
                    {linkedAudits.map(audit => (
                        <div key={audit.id} className="p-5 glass-panel rounded-3xl border border-white/60 dark:border-white/10 shadow-sm hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className={`text-[10px] uppercase font-bold px-2.5 py-1 rounded-lg ${audit.status === 'Terminé' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-500/20' : 'bg-amber-500/15 text-amber-700 dark:text-amber-400 ring-1 ring-amber-500/20'}`}>
                                    {audit.status}
                                </span>
                            </div>
