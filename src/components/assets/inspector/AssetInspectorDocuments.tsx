                    ...
                    {linkedDocuments.map(doc => (
                        <div key={doc.id} className="p-5 glass-panel-lite rounded-3xl border border-white/60 dark:border-white/10 shadow-sm hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <span className={`text-[11px] uppercase font-bold px-2.5 py-1 rounded-lg ${doc.status === 'Publié' ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-500/20' : 'bg-slate-500/10 text-slate-600 dark:text-slate-300 ring-1 ring-slate-500/20'}`}>
                                    {doc.status}
                                </span>
                            </div>
