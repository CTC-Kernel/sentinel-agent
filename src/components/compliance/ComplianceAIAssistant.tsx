...
export const ComplianceAIAssistant: React.FC<ComplianceAIAssistantProps> = ({ control, onApplyPolicy, canEdit = true }) => {
 ...
 <div className="animate-fade-in relative z-10">
  ...
  <button onClick={() => setResponse(null)} className="flex-1 py-2 bg-muted text-muted-foreground rounded-3xl text-xs font-bold hover:bg-muted dark:hover:bg-slate-600 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Retour">
  Retour
  </button>
  {mode === 'policy' && (
  <button onClick={() => onApplyPolicy(response)} className="flex-1 py-2 bg-primary text-primary-foreground rounded-3xl text-xs font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Insérer la politique générée">
