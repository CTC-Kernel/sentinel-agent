        ...
        <div className="space-y-6 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 dark:border-border/40 shadow-sm">
                    <h3 className="text-xs font-bold uppercase text-muted-foreground mb-4 tracking-widest">Actifs Liés</h3>
                        ...
                        {(Array.isArray(control.relatedAssetIds) ? control.relatedAssetIds : []).map(assetId => {
                            ...
                            return asset ? (
                                <div key={assetId || 'unknown'} className="flex items-center justify-between p-2 bg-white/40 dark:bg-white/5 rounded-lg text-sm border border-border/40 shadow-sm">
                                    ...
                                    {canEdit && <Button variant="ghost" size="icon" aria-label="Délier l'actif" onClick={() => handleUnlinkAsset(control, assetId)} disabled={updating} className="h-6 w-6 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:bg-red-900/20"><X className="h-3.5 w-3.5" /></Button>}
                                </div>
                            ) : null;
                        })}
                        {(!control.relatedAssetIds || control.relatedAssetIds.length === 0) && <p className="text-xs text-muted-foreground italic">Aucun actif lié.</p>}
                    </div>
                    {canEdit && (
                        <CustomSelect
                            label=""
                ...
                {/* Suppliers */}
                <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 dark:border-border/40 shadow-sm">
                    <h3 className="text-xs font-bold uppercase text-muted-foreground mb-4 tracking-widest">Fournisseurs Liés</h3>
                    <div className="space-y-2 mb-4">
                        {(Array.isArray(control.relatedSupplierIds) ? control.relatedSupplierIds : []).map(supplierId => {
                            ...
                            return supplier ? (
                                <div key={supplierId || 'unknown'} className="flex items-center justify-between p-2 bg-white/40 dark:bg-white/5 rounded-lg text-sm border border-border/40 shadow-sm">
                                    ...
                                    {canEdit && <Button variant="ghost" size="icon" aria-label="Délier le fournisseur" onClick={() => handleUnlinkSupplier(control, supplierId)} disabled={updating} className="h-6 w-6 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:bg-red-900/20"><X className="h-3.5 w-3.5" /></Button>}
                                </div>
                            ) : null;
                        })}
                        {(!control.relatedSupplierIds || control.relatedSupplierIds.length === 0) && <p className="text-xs text-muted-foreground italic">Aucun fournisseur lié.</p>}
                    </div>
                    {canEdit && (
                        <CustomSelect
                            label=""
            ...
            {/* Projects (Full Width) */}
            <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 dark:border-border/40 shadow-sm">
                <h3 className="text-xs font-bold uppercase text-muted-foreground mb-4 tracking-widest">Projets Liés</h3>
                <div className="space-y-2 mb-4">
                    {(Array.isArray(control.relatedProjectIds) ? control.relatedProjectIds : []).map(pid => {
                        ...
                        return project ? (
                            <div key={pid || 'unknown'} className="flex items-center justify-between p-3 bg-white/40 dark:bg-white/5 rounded-lg text-sm border border-border/40 shadow-sm">
                                ...
                                {canEdit && <Button variant="ghost" size="icon" aria-label="Délier le projet" onClick={() => handleUnlinkProject(control, pid)} disabled={updating} className="h-6 w-6 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:bg-red-900/20"><X className="h-3.5 w-3.5" /></Button>}
                            </div>
                        ) : null;
                    })}
                    {(!control.relatedProjectIds || control.relatedProjectIds.length === 0) && <p className="text-xs text-muted-foreground italic">Aucun projet lié.</p>}
                </div>
                {canEdit && (
                    <CustomSelect
                        label=""
                    ...
                    <div className="space-y-2">
                        {safeRisks.filter(r => r.mitigationControlIds?.includes(control.id)).map(risk => (
                            <div key={risk.id || 'unknown'} className="p-2 bg-white/60 dark:bg-black/20 rounded-lg text-xs border border-red-100 dark:border-red-900/30">
                                ...
                                <div className="text-muted-foreground">Risque brut: {risk.score}</div>
                            </div>
                        ))}
                    ...
                    <div className="space-y-2">
                        {safeFindings.filter(f => f.relatedControlId === control.id && f.status === 'Ouvert').map(finding => (
                            <div key={finding.id || 'unknown'} className="p-2 bg-white/60 dark:bg-black/20 rounded-lg text-xs border border-yellow-100 dark:border-yellow-900/30">
                                ...
                                <div className="text-muted-foreground">Type: {finding.type}</div>
                            </div>
                        ))}
