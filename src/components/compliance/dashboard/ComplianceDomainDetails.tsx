...
export const ComplianceDomainDetails: React.FC<ComplianceDomainDetailsProps> = ({ controls, currentFramework }) => {
 ...
 {Object.entries(domainData).map(([domain, data]) => {
  ...
  return (
  <div key={domain || 'unknown'} className="p-4 bg-white/40 dark:bg-white/5 rounded-3xl border border-border/40 backdrop-blur-sm hover:scale-[1.02] transition-transform duration-200">
  ...
  <div className="w-full bg-muted dark:bg-white/10 rounded-full h-2 mb-2 overflow-hidden">
  <div
