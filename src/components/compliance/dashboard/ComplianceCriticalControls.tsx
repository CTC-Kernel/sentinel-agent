...
export const ComplianceCriticalControls: React.FC<ComplianceCriticalControlsProps> = ({ controls }) => {
 ...
 {criticalControls.slice(0, 5).map((control, index) => (
  <div key={`task-${index || 'unknown'}`} className="flex items-center justify-between p-4 bg-white/50 dark:bg-white/5 rounded-2xl border border-border/40 hover:bg-white/70 dark:hover:bg-white/10 transition-colors shadow-sm shadow-black/5">
