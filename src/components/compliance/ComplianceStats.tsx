...
export const ComplianceStats: React.FC<ComplianceStatsProps> = ({ controls, onFilterChange }) => {
    ...
    const chartTheme = {
        ...
        colors: {
            implemented: '#10b981', // Emerald 500
            partial: '#f59e0b',     // Amber 500
            notStarted: '#f43f5e',  // Rose 500
        ...
        const fetchTrend = async () => {
            ...
            try {
                ...
                if (history.length >= 2) {
                    ...
                    const previous = history[0].metrics.complianceRate;
                    setTrend(Math.round(current - previous));
                }
            } catch (error) {
                ErrorLogger.error(error, 'ComplianceStats.fetchTrend');
            }
        };
        fetchTrend();
    }, [user?.organizationId]);
