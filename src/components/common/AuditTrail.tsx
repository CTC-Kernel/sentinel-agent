...
export const AuditTrail: React.FC<AuditTrailProps> = ({ resourceId, className }) => {
 ...
 return (
 <div className="flex flex-col items-center justify-center p-12 text-muted-foreground border border-dashed border-border/40 rounded-3xl">
 ...
 <History className="h-4 w-4 text-primary" /> Piste d'Audit ({resourceLogs.length})
 </h3>

 <div className="relative border-l-2 border-border/40 ml-3 space-y-6 sm:space-y-8">
 {resourceLogs.map((log) => (
  <div key={log.id || 'unknown'} className="relative pl-6">
  {/* Dot */}
  <div className="absolute -left-[9px] top-1 h-4 w-4 rounded-full bg-card border-2 border-border/40 shadow-sm" />

  {/* Header */}
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-2">
  <div className="flex items-center gap-2">
  <span className="text-xs font-bold text-primary bg-primary/10 dark:bg-primary px-2 py-0.5 rounded-full uppercase tracking-wider">
   {log.action}
  </span>
  ...
  {/* Content */}
  <div className="bg-muted/50 dark:bg-white/5 border border-border/40 rounded-lg p-3 text-sm">
