            ...
            <div className="bg-white dark:bg-slate-800 border border-border/40 dark:border-white/5 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative group">
                ...
                <div className="mt-4 flex -space-x-2 overflow-hidden">
                    {group.members?.slice(0, 5).map(memberId => {
                        ...
                        return (
                            <div key={memberId || 'unknown'} className="relative inline-block h-8 w-8 rounded-full ring-2 ring-white dark:ring-slate-800 bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-xs font-medium text-slate-600 dark:text-slate-300 overflow-hidden" title={member.displayName}>
