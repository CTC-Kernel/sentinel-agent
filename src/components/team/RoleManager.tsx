...
const createRoleSchema = (t: (key: string, options?: { defaultValue: string }) => string) => z.object({
 name: z.string().min(2, t('team.roles.validation.nameRequired', { defaultValue: 'Nom requis (min. 2 caractères)' })).max(100, t('team.roles.validation.nameTooLong', { defaultValue: 'Le nom est trop long' })),
 description: z.string().max(500, t('team.roles.validation.descriptionTooLong', { defaultValue: 'La description est trop longue' })).optional(),
...
export const RoleManager: React.FC<RoleManagerProps> = ({ roles, onRefresh }) => {
 ...
 <div>
  <h3 className="text-lg font-bold text-foreground">{t('team.roles.title', { defaultValue: 'Rôles Personnalisés' })}</h3>
  <p className="text-sm text-muted-foreground">{t('team.roles.subtitle', { defaultValue: 'Gérez les permissions fines pour votre organisation.' })}</p>
 </div>
 <Button
  onClick={handleNewRole}
  className="flex items-center gap-2 bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary/90"
 >
 ...
 <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
  ...
  <h4 className="font-bold text-foreground mb-4">{t('team.roles.permissionsMatrix', { defaultValue: 'Matrice des Permissions' })}</h4>
  ...
  <tbody className="divide-y divide-border dark:divide-white/5">
   ...
   <tr key={resource || 'unknown'} className="hover:bg-muted/50 dark:hover:bg-muted/50 transition-colors">
  ...
  <Button type="submit" disabled={isSubmitting} className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20">
