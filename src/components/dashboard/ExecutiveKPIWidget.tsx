...
export function ExecutiveKPIWidget({
	...
	if (hasError) {
		return (
			<ErrorState
				message={errorMessage || t('dashboard.unableToLoadKPIs', { defaultValue: 'Impossible de charger les KPIs' })}
				onRetry={refetchAll}
				t={t}
			/>
		);
	...
	const risksSubtitle =
		risksValue === 0
			? t('dashboard.noRiskCritical', { defaultValue: 'Aucun risque critique' })
			: risksValue === 1
				? t('dashboard.oneNeedsAttention', { defaultValue: '1 nécessite votre attention' })
				: t('dashboard.needsAttention', { defaultValue: '{{count}} nécessitent votre attention', count: risksValue });

	// Generate subtitle for audits
	const auditsSubtitle =
		auditsValue === 0
			? t('dashboard.noControlOngoing', { defaultValue: 'Aucun contrôle en cours' })
			: auditsValue === 1
				? t('dashboard.oneVerificationOngoing', { defaultValue: '1 vérification en cours' })
				: t('dashboard.verificationsOngoing', { defaultValue: '{{count}} vérifications en cours', count: auditsValue });
	...
	return (
		<div
			...
			role="region"
			aria-label={t('dashboard.kpiIndicators', { defaultValue: 'Indicateurs clés de performance' })}
		>
