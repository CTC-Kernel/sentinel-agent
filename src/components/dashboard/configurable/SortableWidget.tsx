...
export const SortableWidget = ({ widget, isEditing, children, onRemove }: SortableWidgetProps) => {
 ...
 {isDragging ? (
 <div className="h-full w-full rounded-4xl border-2 border-dashed border-primary/40 dark:border-primary bg-primary/15 dark:bg-primary/50 backdrop-blur-sm flex items-center justify-center animate-pulse">
  <span className="text-sm font-semibold text-primary">Déplacer ici</span>
 </div>
 ) : (
 <div className={`h-full ring-offset-2 ring-offset-background transition-all duration-200 ${isEditing ? 'ring-2 ring-slate-200 dark:ring-slate-700 rounded-4xl hover:ring-primary/80 cursor-grab active:cursor-grabbing' : ''}`}>
  {children}
 </div>
 )}
