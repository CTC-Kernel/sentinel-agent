...
export const PriorityRisksWidget: React.FC<PriorityRisksWidgetProps> = React.memo(({ topRisks, loading, navigate, t, title }) => {
 ...
 <div className="p-6 space-y-3 h-full overflow-y-auto custom-scrollbar">
  ...
  <div className="w-12 h-12 rounded-3xl bg-gradient-to-br from-error-bg to-warning-bg dark:from-error/10 dark:to-warning/10 border border-error-border dark:border-destructive/20 flex items-center justify-center mr-4 text-error-text dark:text-error font-black text-lg shadow-inner">{risk.score}</div>
  <div>
  ...
  <span className="text-xs font-bold bg-background/60 px-3 py-1.5 rounded-lg border border-border text-muted-foreground">{risk.strategy}</span>
  </div>
  ))}
