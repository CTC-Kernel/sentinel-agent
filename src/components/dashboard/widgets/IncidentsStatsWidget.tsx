...
export const IncidentsStatsWidget: React.FC<IncidentsStatsWidgetProps> = ({ navigate }) => {
 ...
 <div className="grid grid-cols-2 gap-3 mt-auto">
  ...
  className="rounded-3xl bg-gradient-to-br from-success-bg to-success-bg/50 dark:from-success/5 dark:to-success/10 border border-success-border/50 dark:border-success/20 p-3 flex flex-col justify-between hover:scale-[1.02] transition-transform cursor-pointer group/card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success"
  onClick={() => {
  if (navigate) navigate('/incidents');
  }}
  >
  <div className="flex items-center justify-between mb-2">
  <span className="text-xs font-bold uppercase tracking-widest text-success-text dark:text-success">MTTR</span>
  <Clock className="h-3.5 w-3.5 text-success group-hover/card:rotate-12 transition-transform" />
  </div>
  <div>
  <p className="text-lg font-black text-foreground dark:text-success/90 leading-none">
  {stats.avgMttrHours !== null ? `${stats.avgMttrHours}h` : '-'}
  </p>
  <p className="text-xs font-bold text-success-text/70 dark:text-success/70 mt-0.5 uppercase tracking-wider">moyen</p>
  </div>
  </div>
  ...
  className="rounded-3xl bg-gradient-to-br from-warning-bg to-warning-bg/50 dark:from-warning/5 dark:to-warning/10 border border-warning-border/50 dark:border-warning/20 p-3 flex flex-col justify-between hover:scale-[1.02] transition-transform cursor-pointer group/card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning"
  onClick={() => {
  if (navigate) navigate('/incidents');
  }}
  >
  <div className="flex items-center justify-between mb-2">
  <span className="text-xs font-bold uppercase tracking-widest text-warning-text dark:text-warning">Critiques</span>
  <AlertTriangle className="h-3.5 w-3.5 text-warning group-hover/card:shake transition-transform" />
  </div>
  <div>
  <p className="text-lg font-black text-foreground dark:text-warning/90 leading-none">
  {stats.criticalRatio}%
  </p>
  <p className="text-xs font-bold text-warning-text/70 dark:text-warning/70 mt-0.5 uppercase tracking-wider">du total</p>
  </div>
  </div>
 </div>
 </div>
