...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
    ...
    const getWeatherIcon = () => {
        ...
        if (!weather) return <Sun className={`${baseClass} text-warning`} />;
        switch (weather.icon) {
            case 'sun': return <Sun className={`${baseClass} text-warning animate-pulse-slow`} />;
            case 'cloud': return <Cloud className={`${baseClass} text-muted-foreground`} />;
            case 'rain': return <CloudRain className={`${baseClass} text-info-text`} />;
            case 'snow': return <CloudSnow className={`${baseClass} text-info-text`} />;
            case 'storm': return <CloudLightning className={`${baseClass} text-violet-500`} />;
            default: return <Sun className={`${baseClass} text-warning`} />;
        }
    };
    ...
    const CARD_STYLES: Record<string, {
        ...
        purple: {
            bg: 'bg-muted',
            bgHover: 'group-hover/card:bg-muted dark:group-hover/card:bg-muted',
                                    ...
                                    {isGettingStartedClosed && onShowGettingStarted && (
                                        <Tooltip content={t('dashboard.showGettingStarted')} position="bottom">
                                            <button
                                                ...
                                                className="p-2.5 rounded-2xl text-muted-foreground hover:text-warning hover:bg-warning-bg transition-all duration-300"
   ...
   ${insight.type === 'danger'
                                            ...
                                            : insight.type === 'warning'
                                                ? 'bg-warning-bg/40 border-warning-border/30 hover:bg-warning-bg/60 hover:border-warning-border/50'
                                                : 'bg-success-bg/40 border-success-border/30 hover:bg-success-bg/60 hover:border-success-border/50'
                                        }`}
                                >
                                    <div className={`p-3 rounded-2xl shrink-0 transition-all duration-500 group-hover/insight:scale-110 shadow-sm
   ${insight.type === 'danger' ? 'bg-error-bg' : 'bg-success-bg'}`}>
                                        {insight.type === 'danger' ? (
                                            <AlertTriangle className="h-6 w-6 text-error-text animate-pulse-subtle" />
                                        ) : (
                                            <ShieldCheck className="h-6 w-6 text-success-text" />
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                    ...
                                    <div className="p-2 rounded-xl bg-white/20 dark:bg-white/10 opacity-0 group-hover/insight:opacity-100 transition-all duration-300 translate-x-1 group-hover/insight:translate-x-0">
