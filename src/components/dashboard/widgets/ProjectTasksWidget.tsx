...
export const ProjectTasksWidget: React.FC<ProjectTasksWidgetProps> = ({ navigate, t = (k) => k }) => {
 ...
 <div className="bg-muted/50 p-2 rounded-lg text-center">
  <div className="text-xl font-bold text-foreground">{projectStats.activeCount}</div>
 ...
 {projectStats.urgentProjects.map(project => {
  ...
  className="group flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer border border-transparent hover:border-border/40 dark:hover:border-slate-700"
  onClick={() => {
  ...
  <div className="ml-2 w-8 h-8 rounded-full flex items-center justify-center bg-muted text-xs font-bold text-muted-foreground">
  {project.progress}%
  </div>
  </div>
  );
