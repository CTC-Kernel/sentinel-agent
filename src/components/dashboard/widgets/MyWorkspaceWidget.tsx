...
export const MyWorkspaceWidget: React.FC<MyWorkspaceWidgetProps> = React.memo(({ myActionItems, loading, navigate, t }) => {
 ...
 {loading ? <Skeleton className="h-full w-full m-4" /> : myActionItems.length === 0 ? (
  <div className="flex flex-col items-center justify-center h-full p-8 text-center"><CheckCircle2 className="h-12 w-12 text-success/30 mb-4" /><p className="text-sm font-bold text-muted-foreground">{t('dashboard.nothingToReport')}</p></div>
 ) : (
  ...
  >
  <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${item.type === 'audit' ? 'bg-info' :
   item.type === 'policy' ? 'bg-destructive' :
   item.type === 'incident' ? 'bg-destructive' :
   item.type === 'risk' ? 'bg-warning' :
   'bg-warning'
   }`}></div>
  <div className="flex-1 min-w-0">
   ...
   item.type === 'incident' ? 'bg-error-bg border-error-border text-error-text dark:bg-error/10 dark:border-error/20 dark:text-error' :
    ...
    'bg-warning-bg border-warning-border text-warning-text dark:bg-warning/10 dark:border-warning/20 dark:text-warning'
   }`}>
   ...
   <h4 className="text-sm font-bold text-foreground group-hover/item:text-primary dark:group-hover/item:text-primary/70 transition-colors truncate">{item.title}</h4>
   <p className="text-xs text-muted-foreground mt-0.5 truncate">{item.status}</p>
  </div>
  <ArrowRight className="w-4 h-4 text-muted-foreground group-hover/item:text-foreground -translate-x-2 opacity-0 group-hover/item:translate-x-0 group-hover/item:opacity-70 transition-all" />
  </div>
  ))}
  </div>
 )}
