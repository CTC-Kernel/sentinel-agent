...
export const TopRisksWidget: React.FC<TopRisksWidgetProps> = ({ risks, onMitigate }) => {
 ...
 {topRisks.length === 0 ? (
  ...
  <p>{t('dashboard.noHighRisk', { defaultValue: 'Aucun risque élevé identifié.' })}</p>
  <p className="text-xs mt-1">{t('dashboard.excellentSecurity', { defaultValue: 'Excellente posture de sécurité !' })}</p>
  </div>
 ) : (
  topRisks.map(risk => (
