...
export const HunterProfileModal: React.FC<HunterProfileModalProps> = ({ isOpen, onClose, hunterName }) => {
 ...
 <div className="fixed inset-0 overflow-y-auto">
  ...
  >
  <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-3xl bg-card p-0 text-left align-middle shadow-2xl transition-all">
  <div className="relative">
   ...
   (hunterProfile?.user?.displayName || hunterName) === 'CyberAlliance' ? 'bg-gradient-to-br from-muted-foreground/30 to-muted/500' : 
   ...
   {(mockHunterProfiles[hunterProfile?.user?.displayName || hunterName]?.rank || 4) <= 3 && (
    ...
    (mockHunterProfiles[hunterProfile?.user?.displayName || hunterName]?.rank || 4) === 2 ? 'text-muted-foreground' : 
    'text-orange-700'
    }`} />
    </div>
   )}
   ...
   <div className="text-sm text-muted-foreground">{t('threatIntel.hunterProfile.ranking', { defaultValue: 'Classement' })}</div>
   </div>
   </div>
   ...
   <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
    <div className="bg-muted/50 rounded-2xl p-4 border border-border/40 dark:border-white/5">
    <div className="flex items-center gap-2 mb-2">
    <Activity className="h-4 w-4 text-primary" />
    <div className="text-xs uppercase tracking-wider text-muted-foreground">{t('threatIntel.hunterProfile.contributions', { defaultValue: 'Contributions' })}</div>
    </div>
    <div className="text-2xl font-black text-foreground">{hunterProfile?.stats?.totalContributions || 0}</div>
    </div>
    <div className="bg-muted/50 rounded-2xl p-4 border border-border/40 dark:border-white/5">
    <div className="flex items-center gap-2 mb-2">
    <Shield className="h-4 w-4 text-emerald-500" />
    <div className="text-xs uppercase tracking-wider text-muted-foreground">{t('threatIntel.hunterProfile.verified', { defaultValue: 'Verifiées' })}</div>
    </div>
    <div className="text-2xl font-black text-foreground">{hunterProfile?.stats?.verifiedThreats || 0}</div>
    </div>
    <div className="bg-muted/50 rounded-2xl p-4 border border-border/40 dark:border-white/5">
    <div className="flex items-center gap-2 mb-2">
    <Zap className="h-4 w-4 text-yellow-500" />
    <div className="text-xs uppercase tracking-wider text-muted-foreground">{t('threatIntel.hunterProfile.response', { defaultValue: 'Réponse' })}</div>
    </div>
    <div className="text-2xl font-black text-foreground">{hunterProfile?.stats?.averageResponseTime || 0} min</div>
    </div>
    <div className="bg-muted/50 rounded-2xl p-4 border border-border/40 dark:border-white/5">
    <div className="flex items-center gap-2 mb-2">
    <TrendingUp className="h-4 w-4 text-purple-500" />
    <div className="text-xs uppercase tracking-wider text-muted-foreground">{t('threatIntel.hunterProfile.thisMonth', { defaultValue: 'Ce mois' })}</div>
    </div>
    <div className="text-2xl font-black text-foreground">{stats.threatsThisMonth}</div>
    </div>
   </div>
   ...
   <div>
    ...
    {t('threatIntel.hunterProfile.expertise', { defaultValue: 'Expertise' })}
    </h3>
   ...
   <div>
    ...
    {t('threatIntel.hunterProfile.recentActivity', { defaultValue: 'Activité Récente' })}
    </h3>
    <div className="space-y-3">
    {recentActivity.map((activity: RecentActivity, index: number) => (
    <div key={index || 'unknown'} className="flex items-start gap-3 p-3 bg-muted/50 rounded-3xl border border-border/40 dark:border-white/5">
    ...
    <div className="flex-1">
     ...
     <div className="text-xs text-muted-foreground mt-1">{activity.impact}</div>
    </div>
    </div>
    ))}
    </div>
   ...
   {/* Performance Metrics */}
   <div className="bg-gradient-to-br from-primary/10 to-purple-50 dark:from-primary/20 dark:to-purple-900/20 rounded-2xl p-6 border border-primary/30 dark:border-primary/90">
    <h3 className="text-lg font-bold text-foreground mb-4">{t('threatIntel.hunterProfile.performance', { defaultValue: 'Performance' })}</h3>
    <div className="space-y-4">
    <div>
    <div className="flex justify-between text-sm mb-1">
    <span className="text-muted-foreground">{t('threatIntel.hunterProfile.accuracy', { defaultValue: 'Précision' })}</span>
    <span className="font-bold text-foreground">{stats.accuracyRate}%</span>
    </div>
    ...
    <div>
    <div className="flex justify-between text-sm mb-1">
    <span className="text-muted-foreground">{t('threatIntel.hunterProfile.reputation', { defaultValue: 'Réputation' })}</span>
    <span className="font-bold text-foreground">{stats.reputationScore}/5.0</span>
    </div>
    <div className="w-full bg-muted rounded-full h-2">
    <div 
    ...
    <span className="text-muted-foreground">{t('threatIntel.hunterProfile.collaborations', { defaultValue: 'Collaborations' })}</span>
    <span className="font-bold text-foreground">{stats.collaborations}</span>
    </div>
    </div>
    </div>
   </div>
   ...
   {achievements.length > 0 && (
    ...
    {t('threatIntel.hunterProfile.achievements', { defaultValue: 'Réalisations' })}
    </h3>
    <div className="space-y-3">
   ...
   {/* Member Since */}
   <div className="text-center p-4 bg-muted/50 rounded-3xl border border-border/40 dark:border-white/5">
    <div className="text-sm text-muted-foreground mb-1">{t('threatIntel.hunterProfile.memberSince', { defaultValue: 'Membre depuis' })}</div>
    <div className="font-bold text-foreground">{new Date(hunterProfile?.user?.createdAt || '2023-01-01').toLocaleDateString(config.intlLocale, { year: 'numeric', month: 'long' })}</div>
   </div>
   </div>
   </div>
   </div>
