...
const RiskReduction: React.FC<RiskReductionProps> = ({ percentage }) => {
  ...
  return (
    <div className="flex items-center gap-1 text-green-600 dark:text-green-400">
      ...
      <span className="text-xs text-slate-500 ml-1">{t('vendorConcentration.recommendations.riskReduction')}</span>
    </div>
  );
};
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <ChevronRight
            className={`h-5 w-5 text-slate-400 transition-transform ml-4 ${expanded ? 'rotate-90' : ''}`}
          />
        </div>
      </button>
        ...
        <div className="border-t border-slate-200 dark:border-slate-700 p-4 space-y-4">
          ...
          {recommendation.actions.length > 0 && (
            <div>
              ...
              {/* Progress bar */}
              <div className="h-1.5 rounded-full bg-slate-200 dark:bg-slate-700 mb-3 overflow-hidden">
                <div
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-6 p-4 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-slate-500">{t('vendorConcentration.recommendations.totalReduction')}</p>
        </div>
      </div>
