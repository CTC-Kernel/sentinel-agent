...
export const PrivacyRequestInspector: React.FC<PrivacyRequestInspectorProps> = ({
 ...
 icon={User}
 >
 <div className="p-6 md:p-8 space-y-8 bg-muted/50 dark:bg-transparent min-h-full">
 {/* Summary Card */}
 <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 shadow-sm relative overflow-hidden">
  ...
  <div>
   <div className="text-lg font-bold text-foreground">{request.dataSubject}</div>
  ...
  <div className="flex justify-between items-center p-3 bg-card/40 backdrop-blur-sm rounded-3xl border border-border/40">
   <span className="text-sm font-medium text-muted-foreground">{t('privacy.request.type', { defaultValue: 'Type' })}</span>
   <span className="font-bold text-primary">{request.requestType}</span>
  </div>
  <div className="flex justify-between items-center p-3 bg-card/40 backdrop-blur-sm rounded-3xl border border-border/40">
   <span className="text-sm font-medium text-muted-foreground">{t('privacy.request.priority', { defaultValue: 'Priorité' })}</span>
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  ...
  <div className="text-xl font-bold text-foreground">{format(new Date(request.submissionDate), 'dd MMMM yyyy', { locale: dateFnsLocale })}</div>
  </div>
  ...
  <div className="text-xl font-bold text-foreground">{format(new Date(request.dueDate), 'dd MMMM yyyy', { locale: dateFnsLocale })}</div>
  </div>
 ...
 <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 shadow-sm">
  ...
  <div className="relative pl-8 border-l-2 border-border/40 space-y-6 sm:space-y-8">
  ...
  <div key={step || 'unknown'} className="relative">
   <div className={`absolute -left-[41px] top-0 h-5 w-5 rounded-full border-2 flex items-center justify-center bg-card/80 backdrop-blur-sm ${isCompleted ? 'border-primary text-primary' : 'border-border/40 text-transparent'
   }`}>
   <div className={`h-2.5 w-2.5 rounded-full ${isCompleted ? 'bg-primary' : 'bg-muted'}`}></div>
   </div>
   <h5 className={`text-sm font-bold ${isCompleted ? 'text-foreground' : 'text-muted-foreground'}`}>
   ...
   className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-3xl text-sm font-bold transition-all shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
   >
   ...
   disabled={isHolding}
   className="px-4 py-2 bg-muted hover:bg-muted text-foreground rounded-3xl text-sm font-bold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
   >
