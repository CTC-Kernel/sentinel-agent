...
export const PrivacyRequests: React.FC<PrivacyRequestsProps> = ({ onCreate, onSelect }) => {
 ...
 <div className="relative w-full md:w-96">
  ...
  <input aria-label={t('privacy.requests.searchLabel', { defaultValue: 'Rechercher une demande' })}
  type="text"
  placeholder={t('privacy.requests.searchPlaceholder', { defaultValue: 'Rechercher une demande (Nom, ID)...' })}
  value={searchTerm}
  onChange={(e) => setSearchTerm(e.target.value)}
  className="w-full pl-10 pr-4 py-2 bg-card border border-border/40 rounded-3xl text-sm focus-visible:ring-2 focus-visible:ring-primary outline-none"
  />
 </div>
 <div className="flex gap-2 w-full md:w-auto">
  ...
  className="px-4 py-2 bg-card border border-border/40 rounded-3xl text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-primary"
  >
