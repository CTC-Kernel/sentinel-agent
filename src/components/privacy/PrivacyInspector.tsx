...
export const PrivacyInspector: React.FC<PrivacyInspectorProps> = ({
 ...
 <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-8 bg-muted/50 dark:bg-transparent custom-scrollbar">
 {inspectorTab === 'details' && (
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
   <div>
   <FloatingLabelInput label={t('privacy.inspector.name', { defaultValue: 'Nom' })} {...editActivityForm.register('name')} />
   </div>
   <div>
   <div className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">{t('privacy.fields.manager', { defaultValue: 'Responsable' })}</div>
   <CustomSelect
  ...
  <FloatingLabelInput label={t('privacy.inspector.purpose', { defaultValue: 'Finalité' })} textarea rows={3} {...editActivityForm.register('purpose')} />

  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
   <div>
   <label htmlFor="legalBasis" className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">{t('privacy.fields.legalBasis', { defaultValue: 'Base Légale' })}</label>
   <CustomSelect
   id="legalBasis"
   ...
   <div>
   <label htmlFor="status" className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">{t('privacy.fields.status', { defaultValue: 'Statut' })}</label>
   <CustomSelect
   id="status"
   value={watchedStatus}
 ...
 {inspectorTab === 'history' && (
  ...
  <span className="absolute -left-[41px] top-1 flex h-5 w-5 items-center justify-center rounded-full bg-card/80 backdrop-blur-sm border-2 border-primary/30 dark:border-primary/90">
  ...
  <div>
   ...
   <p className="text-[11px] text-muted-foreground mt-1">{t('privacy.history.by', { defaultValue: 'Par' })}: {log.userEmail}</p>
  </div>
  </div>
  ))}
 ...
 {inspectorTab === 'comments' && (
  ...
  title={t('privacy.inspector.discussionTitle', { defaultValue: 'Discussion - {{name}}', name: selectedActivity.name })}
  enableSearch={true}
  enableFilters={true}
  enableExport={true}
