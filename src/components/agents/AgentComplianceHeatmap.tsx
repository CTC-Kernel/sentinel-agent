...
// Get status label for tooltip - uses t function from caller
const getStatusLabel = (status: CheckStatus, t: (key: string, options?: { defaultValue: string }) => string): string => {
 switch (status) {
 case 'pass': return t('agent.status.compliant', { defaultValue: 'Conforme' });
 case 'fail': return t('agent.status.nonCompliant', { defaultValue: 'Non conforme' });
 case 'pending': return t('agent.status.pending', { defaultValue: 'En attente' });
 default: return t('agent.status.notChecked', { defaultValue: 'Non vérifié' });
 }
};
...
const HeatmapCell: React.FC<HeatmapCellProps> = ({ status, agentName, checkName, onClick, t }) => {
 ...
 aria-label={`${checkName} - ${agentName}: ${getStatusLabel(status, t)}`}
 className={cn(
...
const SummaryStats: React.FC<SummaryStatsProps> = ({ agents, checks, getStatus, t }) => {
 const stats = useMemo(() => {
 let pass = 0;
 let fail = 0;
 let pending = 0;
 ...
 <span className="text-muted-foreground">{t('agent.status.compliant', { defaultValue: 'Conforme' })}</span>
 ...
 <span className="text-muted-foreground">{t('agent.status.nonCompliant', { defaultValue: 'Non conforme' })}</span>
 ...
 <span className="text-muted-foreground">{t('agent.status.pending', { defaultValue: 'En attente' })}</span>
 <span className="font-bold text-foreground">{stats.pending}</span>
 </div>
 <div className="ml-auto flex items-center gap-2">
 <span className="text-muted-foreground">{t('agent.complianceRate', { defaultValue: 'Taux de conformité:' })}</span>
 <Badge
...
export const AgentComplianceHeatmap: React.FC<AgentComplianceHeatmapProps> = ({
 ...
 <h3 className="text-lg font-semibold text-foreground mb-1">
  {t('agent.noComplianceData', { defaultValue: 'Aucune donnée de conformité' })}
 </h3>
 <p className="text-sm text-muted-foreground max-w-sm">
  {t('agent.deployAgentsForMatrix', { defaultValue: 'Déployez des agents pour voir la matrice de conformité.' })}
 </p>
 </motion.div>
 );
 }
 ...
 <div className="flex items-center justify-between mb-3">
  <div>
  <h3 className="text-sm font-bold text-foreground">{t('agent.complianceMatrix', { defaultValue: 'Matrice de Conformité' })}</h3>
  <p className="text-xs text-muted-foreground mt-0.5">
  {t('agent.checksOverviewByAgent', { defaultValue: "Vue d'ensemble des checks par agent" })}
  </p>
  </div>
  <Tooltip
  content={t('agent.complianceMatrixTooltip', { defaultValue: "Cette matrice affiche l'état de conformité de chaque agent pour les différents contrôles de sécurité." })}
  position="left"
  >
 ...
 <SummaryStats agents={displayedAgents} checks={DEFAULT_CHECKS} getStatus={getStatus} t={t} />
 </div>
 ...
 <table className="w-full">
  ...
  title={t('agent.clickToViewDetails', { defaultValue: "Cliquez pour voir les details de l'agent" })}
 ...
 <div className="px-5 py-3 border-t border-border/30 bg-muted/10">
  ...
  >
  {showAll
  ? t('agent.showLess', { defaultValue: 'Afficher moins' })
  : t('agent.showMoreAgents', { defaultValue: `Afficher ${agents.length - maxAgentsDisplay} agents supplémentaires` })
  }
  </Button>
 </div>
 )}
