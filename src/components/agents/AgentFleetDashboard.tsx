...
export const AgentFleetDashboard: React.FC<AgentFleetDashboardProps> = ({ agents, loading, complianceResults }) => {
 ...
 <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
  ...
  {t('agent.deployedEndpoints', { defaultValue: 'endpoints déployés' })}
  </span>
  </div>
  ...
  <span className="text-muted-foreground">
   <strong className="text-foreground">{stats.active}</strong> {t('agent.activeStatus', { defaultValue: 'actifs' })}
  </span>
  </div>
  <div className="flex items-center gap-2">
  <span role="img" aria-hidden="true" className="w-2 h-2 rounded-full bg-muted-foreground" />
  <span className="text-muted-foreground">
   <strong className="text-foreground">{stats.offline}</strong> {t('agent.offlineStatus', { defaultValue: 'hors ligne' })}
  </span>
  </div>
  {stats.critical > 0 && (
  <div className="flex items-center gap-2" role="alert">
   ...
   {stats.critical} {t('agent.criticalStatus', { defaultValue: 'critiques' })}
   </span>
  </div>
  )}
  </div>
 ...
 <motion.div variants={slideUpVariants}>
  <StatCard
  label={t('agent.activityRate', { defaultValue: "Taux d'activité" })}
 ...
 <motion.div variants={slideUpVariants}>
  <StatCard
  label={t('agent.complianceScore', { defaultValue: 'Score Conformité' })}
  value={stats.avgScore !== null ? `${stats.avgScore}%` : '-'}
  sublabel={stats.avgScore !== null ? t('agent.fleetAverage', { defaultValue: 'Moyenne de la flotte' }) : t('agent.noData', { defaultValue: 'Aucune donnée' })}
 ...
 <motion.div variants={slideUpVariants}>
  <StatCard
  label={t('agent.upToDateAgents', { defaultValue: 'Agents à jour' })}
 ...
 <motion.div variants={slideUpVariants}>
  <StatCard
  label={t('agent.criticalAlerts', { defaultValue: 'Alertes Critiques' })}
  value={stats.critical}
  sublabel={t('agent.agentsInAlert', { defaultValue: 'Agents en alerte' })}
 ...
 >
  ...
  <div>
  <h3 className="text-sm font-bold text-foreground">{t('agent.currentComplianceScore', { defaultValue: 'Score Conformité Actuel' })}</h3>
  <p className="text-xs text-muted-foreground">{t('agent.basedOnLatestResults', { defaultValue: 'Basé sur les derniers résultats remontés' })}</p>
  </div>
  <div className="flex items-center gap-2 text-xs text-muted-foreground">
  <div className="w-3 h-3 rounded-full bg-primary" />
  <span>{t('agent.complianceScoreLabel', { defaultValue: 'Score conformité' })}</span>
  </div>
  </div>
  ...
  <p>{t('agent.insufficientDataForTrend', { defaultValue: 'Données insuffisantes pour afficher une tendance' })}</p>
  </div>
  </div>
  ) : (
