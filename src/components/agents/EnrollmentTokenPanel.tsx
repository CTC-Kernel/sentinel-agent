...
export const EnrollmentTokenPanel: React.FC<EnrollmentTokenPanelProps> = ({
 ...
 <div className="p-4 bg-card rounded-3xl border border-border/40 shadow-sm">
  <div className="font-mono text-xs text-primary break-all select-all text-center mb-3">
  {enrollmentToken}
  </div>
  <Button
  size="sm"
 ...
 <div className="text-xs text-muted-foreground space-y-2">
  <p><strong className="text-foreground">macOS / Linux :</strong> {t('agentSetup.tokenReady.platformMacLinux', { defaultValue: 'Ouvrez un terminal' })}</p>
  <p><strong className="text-foreground">Windows :</strong> {t('agentSetup.tokenReady.platformWindows', { defaultValue: 'PowerShell (Admin)' })}</p>
 </div>
 </div>
 </div>
 );
