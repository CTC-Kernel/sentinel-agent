            ...
            {hasMembers ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {safeUsersList.filter(u => project.members?.includes(u.uid)).map(member => (
                        <div key={member.uid} className="glass-premium p-4 rounded-xl border border-border/40 flex items-center gap-4 group hover:bg-white/50 dark:hover:bg-white/5 transition-colors">
