...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
    ...
    return (
        ...
        >
            <h4 className="text-xs font-bold uppercase text-slate-600 dark:text-slate-400 mb-3 flex justify-between tracking-wider">
            ...
            <div className="space-y-2.5 flex-1">
                {tasks.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 dark:text-slate-500 min-h-[150px] border-2 border-dashed border-slate-200 dark:border-white/5 rounded-xl">
                        ...
                        >
                            ...
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-70 transition-opacity">
                                    <button onClick={() => onEditTask(task)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:bg-white/10 rounded text-slate-500 dark:text-slate-400 hover:text-brand-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"><Edit className="h-3.5 w-3.5" /></button>
                                    <button onClick={() => handleDelete(task.id)} disabled={deletingIds.has(task.id)} className="p-1 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:bg-red-900/20 rounded text-slate-500 dark:text-slate-400 hover:text-red-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 disabled:bg-slate-200 disabled:text-slate-500 disabled:border-slate-300 disabled:cursor-not-allowed dark:disabled:bg-slate-700 dark:disabled:text-slate-400 dark:disabled:border-slate-600"><Trash2 className="h-3.5 w-3.5" /></button>
                                </div>
                            </div>
