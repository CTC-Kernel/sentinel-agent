...
export const ProjectCard: React.FC<ProjectCardProps> = ({
    project, canEdit, user, onEdit, onDelete, onClick, onDuplicate
}) => {
    return (
        <div onClick={() => onClick(project)} className="glass-panel rounded-[2.5rem] p-6 card-hover flex flex-col cursor-pointer group border border-white/50 dark:border-white/5">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {canEdit && (
                        <>
                            {/* Copy Icon (Duplicate) - Assuming Copy import exists or I need to add it. */}
                            <button onClick={(e) => { e.stopPropagation(); onDuplicate(project); }} className="p-1.5 bg-white/80 dark:bg-slate-800/80 rounded-lg text-slate-500 hover:text-brand-500 shadow-sm backdrop-blur-sm transition-colors">
                                {/* Use basic copy icon or similar if unavailable, but I should import Copy from lucide or Icons */}
