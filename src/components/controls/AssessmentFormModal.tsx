...
export const AssessmentFormModal: React.FC<AssessmentFormModalProps> = ({
 ...
 <PremiumCard glass className="p-6">
  ...
  <div className="p-5 rounded-3xl border border-warning-border bg-warning-bg/30 space-y-4">
   ...
   className="rounded-2xl bg-primary shadow-lg shadow-primary/20 flex-1"
   >
  ...
  className={`w-full px-4 py-2.5 rounded-3xl border bg-card text-foreground ${validationErrors.controlCode ? 'border-red-500 ring-1 ring-red-500' : 'border-border/40'} ${controls.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
  required
  ...
  className={`w-full px-4 py-2.5 rounded-3xl border bg-card text-foreground ${validationErrors.assessmentMethod ? 'border-red-500 ring-1 ring-red-500' : 'border-border/40'}`}
  ...
  <input
   ...
   className="w-full pl-10 pr-4 py-2.5 rounded-3xl border border-border/40 bg-card text-foreground"
  />
  </div>
  </div>
