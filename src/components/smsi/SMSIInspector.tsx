...
export const SMSIInspector: React.FC<SMSIInspectorProps> = ({
 ...
 <PremiumCard glass className="p-6">
  ...
  <p className="text-foreground">{milestone.description || t('common.noDescription', { defaultValue: 'Aucune description' })}</p>
  </div>
  ...
  <div>
  <span className="text-sm font-medium text-muted-foreground block mb-1">Responsable</span>
  <div className="flex items-center gap-2 text-foreground">
  ...
  <div>
  <span className="text-sm font-medium text-muted-foreground block mb-1">Phase PDCA</span>
  ...
  <span className="text-foreground">{phaseConfig.label} - {phaseConfig.description}</span>
  </div>
  </div>
  </div>
 </PremiumCard>
 ...
 {linkedItems.length > 0 && (
  ...
  <div key={item.id || 'unknown'} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
   <FileText className="w-4 h-4 text-muted-foreground" />
   <span className="text-sm text-foreground text-muted-foreground">{item.title}</span>
 ...
 {(onStatusChange || onEdit || onDelete) && (
  ...
  {onStatusChange && (
  <div>
   <span className="text-sm font-medium text-muted-foreground block mb-2">{t('smsi.inspector.changeStatus', { defaultValue: 'Changer le statut' })}</span>
