...
interface VoxelStudioProps {
  ...
  // Overlay props - flattened or simplified to allow memoization
...
// Extracted Component for Edges
const EdgesWithColor: React.FC<{ color: string }> = React.memo(({ color }) => (
  <Edges threshold={15} color={color} opacity={0.5} transparent />
));

// Extracted Component for Model Geometry
...
const NeonGrid: React.FC = () => {
  ...
  return (
    <group position={[0, -10.2, 0]}>
      <mesh ref={gridRef} rotation={[-Math.PI / 2, 0, 0]} raycast={disableRaycast}>
        <planeGeometry args={[140, 140, 40, 40]} />
        <meshBasicMaterial color="#1d4ed8" wireframe transparent opacity={0.2} />
      </mesh>
      <mesh ref={grid2Ref} rotation={[-Math.PI / 2, 0, Math.PI / 4]} position={[0, -0.1, 0]} raycast={disableRaycast}>
...
const DataFlowParticles: React.FC<{ start: Vector3; end: Vector3; color: string; opacity?: number; speed?: number; size?: number }> = ({ start, end, color, opacity = 0.5, speed = 1, size = 0.4 }) => {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const t = (time * ANIMATION_SPEED_FACTOR + initialOffsets[i]) % 1;
      const point = curve.getPoint(t);
...
const VoxelMesh: React.FC<{
  ...
  return (
    ...
    // Shallow check overlay props if needed, but deep check is expensive.
    // Assuming overlayProps reference stability or ignore
    true
  );
});
...
export const VoxelStudio: React.FC<VoxelStudioProps> = ({
  ...
  const handlePresentationSelect = useCallback((node: VoxelNode) => {
    setSelectedNode(node);
    focusOnCardRef.current = true;
    shouldSnapToTarget.current = true;
    setImpactPosition(node.position);
          ...
          <Suspense fallback={null}>
            <ModelLibraryProvider>
              ...
              <PresentationManager
                ...
                onNodeSelect={handlePresentationSelect}
              />
