...
const MenuBar = ({ editor }: { editor: Editor | null }) => {
 ...
 return (
 <div className="flex flex-wrap gap-1 p-2 border-b border-border/40 bg-muted/50 dark:bg-white/5 rounded-t-xl">
 <ToolbarButton
 ...
 <div className="w-px h-5 bg-muted dark:bg-white/10 mx-1 self-center" />
...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({
 ...
 editorProps: {
 attributes: {
 class: `prose dark:prose-invert max-w-none p-4 min-h-[150px] focus:outline-none text-foreground text-sm ${!isEditable ? 'opacity-70 cursor-not-allowed bg-muted/50 dark:bg-black/10' : ''}`,
 },
 },
 });
 ...
 {label && <label className="text-sm font-medium text-foreground text-muted-foreground">{label}</label>}
 <div className={`
 border rounded-3xl bg-card transition-all overflow-hidden
 ${error
  ? 'border-error-500 ring-1 ring-error-500/20'
  : 'border-border/40 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/60'
 }
 `}>
