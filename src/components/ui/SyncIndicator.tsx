...
export const SyncIndicator: React.FC = () => {
    ...
    return (
        <Tooltip content={`${t('common.synced') || "Synchronisé"} (${lastSynced?.toLocaleTimeString()})`} position="bottom">
            <div className="flex items-center gap-2 px-2 py-1.5 text-emerald-600 dark:text-emerald-400 rounded-full text-xs font-medium hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition-colors cursor-help">
                <Cloud className="h-4 w-4" />
                <span className="hidden lg:inline text-emerald-600/80 dark:text-emerald-400/80">{t('common.saved') || "Sauvegardé"}</span>
            </div>
        </Tooltip>
    );
};
