...
export const OperationProgress: React.FC<OperationProgressProps> = ({
 ...
 const statusColor = isError ? 'text-error-500' : isComplete ? 'text-success-500' : 'text-primary';
 ...
 <div className="flex items-center gap-3">
  <div className={cn('p-2 rounded-3xl', isError ? 'bg-error-100 dark:bg-error-900/30' : isComplete ? 'bg-success-100 dark:bg-success-900/30' : 'bg-primary/15 dark:bg-primary')}>
 ...
 <div className="space-y-2 pt-2 border-t border-border/40">
  ...
  step.status === 'processing' && 'bg-primary/15 dark:bg-primary text-primary',
 ...
 <div className="absolute inset-0 bg-[var(--overlay-bg)] backdrop-blur-[var(--overlay-blur)]" />
 <motion.div
  initial={{ opacity: 0, scale: 0.95 }}
  animate={{ opacity: 1, scale: 1 }}
  className="relative w-full max-w-md bg-card rounded-2xl shadow-2xl border border-border/40 p-6"
 >
  {content}
 </motion.div>
 </div>
 ...
 animate={{ opacity: 1, y: 0 }}
 exit={{ opacity: 0, y: 20 }}
 className="fixed bottom-6 right-6 z-toast w-80 bg-card rounded-2xl shadow-2xl border border-border/40 p-4"
 >
 {content}
 </motion.div>
 );
 ...
 // Inline variant
 return (
 <div className={cn('bg-card rounded-2xl border border-border/40 p-4', className)}>
 {content}
 </div>
 );
};
