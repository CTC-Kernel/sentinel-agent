...
export const CustomSelect: React.FC<CustomSelectProps> = ({
 ...
 return (
  ...
  ${error
   ...
   : open
   ? 'border border-primary bg-white/80 dark:bg-white/5 shadow-lg shadow-primary/25 ring-2 ring-primary/60'
   : 'glass-input'
   }
  `}>
  ...
  <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4">
   <ChevronDown
   className={`h-4 w-4 text-muted-foreground transition-transform duration-200 ${open ? 'rotate-180 text-primary' : ''}`}
   aria-hidden="true"
   />
  </span>
  </ListboxButton>
  ...
  transition
  portal={true}
  className="z-dropdown w-[var(--button-width)] mt-2 max-h-60 overflow-auto rounded-2xl bg-[var(--dropdown-bg)] border border-[var(--dropdown-border)] py-2 text-base shadow-lg focus-visible:outline-none sm:text-sm custom-scrollbar transition duration-200 ease-out data-[closed]:scale-95 data-[closed]:opacity-0"
  >
  {options.map((option, optionIdx) => (
   <ListboxOption
   key={optionIdx || 'unknown'}
   className={({ focus }) =>
   `relative cursor-pointer select-none py-3 pl-10 pr-4 transition-colors ${focus ? 'bg-primary/10/50 text-foreground' : 'text-muted-foreground'
   }`
   }
   value={option.value}
   >
