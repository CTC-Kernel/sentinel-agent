...
export const DocumentUploadWizard: React.FC<DocumentUploadWizardProps> = ({
 ...
 <div className="px-8 py-6 border-b border-border/40 glass-premium backdrop-blur-md relative z-10">
  ...
  <div className={`
   ...
   idx < currentStep ? 'bg-success-500 text-white' : 'bg-muted text-muted-foreground'}
  `}>
   {idx < currentStep ? <CheckCircle2 className="w-4 h-4" /> : idx + 1}
  </div>
  {idx < STEPS.length - 1 && (
   <div className={`w-8 h-0.5 mx-2 rounded-full transition-colors ${idx < currentStep ? 'bg-success-500' : 'bg-muted dark:bg-white/10'}`} />
  )}
  </div>
  ))}
  </div>
 ...
 <div className="flex-1 overflow-y-auto p-8 custom-scrollbar relative z-10">
  ...
  <div className="text-center mb-8">
   <h3 className="text-lg font-bold text-foreground">Téléchargement du fichier</h3>
   <p className="text-sm text-muted-foreground">Choisissez la source de votre document</p>
  </div>

  <div className="grid grid-cols-2 gap-4 mb-6">
   <button
   type="button"
   className={`cursor-pointer p-4 rounded-3xl border-2 transition-all w-full text-left ${storageProvider === 'firebase' ? 'border-primary bg-primary/10 dark:bg-primary' : 'border-border/40 hover:border-primary/30'}`}
   onClick={() => setValue('storageProvider', 'firebase')}
   >
   <UploadCloud className={`w-6 h-6 mb-2 ${storageProvider === 'firebase' ? 'text-primary' : 'text-muted-foreground'}`} />
   ...
   >
   <Link className={`w-6 h-6 mb-2 ${storageProvider !== 'firebase' ? 'text-primary' : 'text-muted-foreground'}`} />
  ...
  <div className="space-y-6 animate-fade-in-right">
  <div className="text-center mb-8">
   <h3 className="text-lg font-bold text-foreground">Informations Générales</h3>
   <p className="text-sm text-muted-foreground">Détails et classification</p>
  </div>
  <FloatingLabelInput
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
   ...
   <div className="flex p-1 bg-muted dark:bg-black/20 rounded-3xl" role="radiogroup" aria-labelledby="status">
   ...
   <button
    ...
    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${status === s ? 'bg-card shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
    role="radio"
  ...
  <div className="space-y-6 animate-fade-in-right">
  <div className="text-center mb-8">
   <h3 className="text-lg font-bold text-foreground">Relations & Contexte</h3>
   <p className="text-sm text-muted-foreground">Liez ce document aux éléments du GRC</p>
  </div>
  ...
  <div className="text-center mb-6">
   ...
   <h3 className="text-xl font-bold text-foreground">Prêt à valider ?</h3>
   <p className="text-sm text-muted-foreground">Vérifiez les informations ci-dessous</p>
  </div>

  <div className="bg-muted/50 dark:bg-white/5 rounded-2xl p-6 border border-border/40 dark:border-white/5 space-y-4">
   ...
   <div>
   <span className="text-muted-foreground block">Dossier</span>
   <span className="font-medium text-foreground text-muted-foreground">
   {folders.find(f => f.id === folderId)?.name || 'Racine'}
   </span>
   </div>
   <div>
   <span className="text-muted-foreground block">Propriétaire</span>
   <span className="font-medium text-foreground text-muted-foreground">
   {users.find(u => u.uid === ownerId)?.displayName || getValues('owner')}
   </span>
   </div>
   <div>
   <span className="text-muted-foreground block">Fichier</span>
   ...
   <div>
   <span className="text-muted-foreground block">Sécurité</span>
   <span className="font-medium text-foreground text-muted-foreground">
 ...
 <div className="px-8 py-6 border-t border-border/40 glass-premium backdrop-blur-md relative z-10 flex justify-between items-center">
  ...
  <Button onClick={nextStep} className="bg-foreground text-background hover:opacity-90">
  Suivant <ChevronRight className="w-4 h-4 ml-2" />
  </Button>
  ) : (
  <Button onClick={handleSubmit(handleFinalSubmit)} isLoading={isLoading} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg shadow-primary/20">
