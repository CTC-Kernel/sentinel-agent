...
export function ClassificationSelector({
 ...
 disabled={disabled}
 className={cn(
 'inline-flex items-center gap-2 justify-between min-w-[160px] rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground',
 className
 )}
 >
 ...
 {t('documents.classification.title', { defaultValue: 'Classification' })}
 {required && <Badge variant="outline" className="text-xs">{t('common.required', { defaultValue: 'Requis' })}</Badge>}
 </DropdownMenuLabel>
 <DropdownMenuSeparator />
 {levels.map((level) => {
 ...
 {t('documents.classification.hint', { defaultValue: 'La classification détermine qui peut accéder au document' })}
 </div>
 </DropdownMenuContent>
 </DropdownMenu>
 ...
 {t('documents.classification.justification.title', { defaultValue: 'Justification requise' })}
 </DialogTitle>
 <DialogDescription>
 {t(
 'documents.classification.justification.description',
 { defaultValue: `La classification "${pendingLevel ? CLASSIFICATION_CONFIG[pendingLevel].label : ''}" nécessite une justification.` }
 )}
 </DialogDescription>
 </DialogHeader>
 ...
 {t('documents.classification.justification.label', { defaultValue: 'Justification' })}
 </Label>
 <Textarea
 id="justification"
 value={justification}
 ...
 placeholder={t(
  'documents.classification.justification.placeholder',
  { defaultValue: 'Expliquez pourquoi ce niveau de classification est nécessaire...' }
 )}
 rows={3}
 />
 </div>
