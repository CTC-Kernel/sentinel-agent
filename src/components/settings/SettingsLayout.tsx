...
export const SettingsLayout: React.FC<SettingsLayoutProps> = ({ currentTab, onTabChange, children }) => {
 ...
 <nav className="sticky top-24 space-y-4 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
  ...
  return (
   ...
   className={cn(
   "relative flex items-center whitespace-nowrap px-4 py-2 rounded-2xl font-medium text-sm transition-all flex-shrink-0 select-none snap-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
  ...
  return (
   ...
   onClick={() => onTabChange(tab.id)}
   className={cn(
   "group flex items-center w-full px-4 py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all text-left relative overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset",
   isActive
