...
export const SSOManager: React.FC = () => {
 ...
 <div className="flex items-center gap-3">
  ...
  <div>
  <h3 className="text-lg font-bold text-foreground">{t('settings.sso.title', { defaultValue: 'Configuration SSO' })}</h3>
 ...
 <div className="relative z-decorator p-6 space-y-6">
 {/* Domain Verification */}
 <div className="space-y-4">
  <h4 className="text-sm font-bold text-foreground flex items-center gap-2">
  <Globe className="w-4 h-4" /> {t('settings.sso.domainTitle', { defaultValue: "Domaine de l'organisation" })}
  </h4>
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
  <FloatingLabelInput
  label={t('settings.sso.domainLabel', { defaultValue: 'Nom de domaine (ex: entreprise.com)' })}
  value={domain}
  onChange={(e) => setDomain(e.target.value)}
  />
  <div className="flex items-center text-xs text-muted-foreground">
  <AlertTriangle className="w-4 h-4 mr-2 text-amber-500" />
  {t('settings.sso.domainVerificationRequired', { defaultValue: 'La vérification du domaine est requise (DNS TXT)' })}
  </div>
  </div>
 </div>
 ...
 <div className="space-y-4">
  <h4 className="text-sm font-bold text-foreground flex items-center gap-2">
  <Lock className="w-4 h-4" /> {t('settings.sso.identityProviders', { defaultValue: "Fournisseurs d'identité" })}
  </h4>
  ...
  className={cn(
   ...
   : 'border-border/40 bg-white/50 dark:bg-white/5 opacity-60 hover:opacity-70'
  )}
 ...
 {settings.allowedProviders?.includes('saml') && (
  <div className="p-4 bg-muted/50 rounded-3xl border border-border/40 space-y-4 animate-fade-in">
  <h5 className="text-xs font-bold uppercase text-muted-foreground">{t('settings.sso.samlConfig', { defaultValue: 'Configuration SAML 2.0' })}</h5>
