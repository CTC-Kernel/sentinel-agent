...
export const RequirementInspector: React.FC<RequirementInspectorProps> = ({
 ...
 className="fixed right-0 top-0 h-full w-full max-w-xl bg-card/95 backdrop-blur-xl shadow-2xl z-sidebar overflow-hidden flex flex-col"
 >
 {/* Header */}
 <div className="flex-shrink-0 px-6 py-4 border-b border-border/40 bg-card/80 backdrop-blur-md">
 <div className="flex items-start justify-between gap-4">
 <div className="flex-1 min-w-0">
  {/* Article Badge */}
  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted border border-border/40 mb-3">
  <FileText className="w-4 h-4 text-muted-foreground" />
  <span className="font-mono font-semibold text-sm text-foreground text-muted-foreground">
 ...
 {linkedControls.length === 0 ? (
  ...
  <div className="w-12 h-12 mx-auto mb-3 rounded-3xl bg-muted flex items-center justify-center">
  ...
  className="w-full p-3 rounded-3xl bg-card/50 border border-border/40 hover:border-border/40 dark:hover:border-white/20 transition-all text-left group"
  whileHover={{ x: 2 }}
  whileTap={{ scale: 0.99 }}
  >
  ...
  <ExternalLink className="w-4 h-4 text-muted-foreground group-hover:text-muted-foreground dark:group-hover:text-muted-foreground flex-shrink-0 ml-3 transition-colors" />
  </div>
  </motion.button>
  ))}
  </div>
