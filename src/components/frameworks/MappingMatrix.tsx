...
const CoverageLegend: React.FC = () => {
 ...
 { status: 'not_assessed', icon: AlertCircle, label: t('mapping.coverage.not_assessed'), color: 'text-muted-foreground bg-muted' },
 ];
...
const StatsSummary: React.FC<StatsSummaryProps> = ({ controls, frameworks }) => {
 ...
 <span className="font-bold text-foreground">{controls.length}</span>
 <span className="text-muted-foreground">{t('mapping.controls')}</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-foreground">{frameworks.length}</span>
 <span className="text-muted-foreground">{t('mapping.frameworks')}</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-emerald-600 dark:text-emerald-400">{stats.fullCoverage}</span>
 <span className="text-muted-foreground">{t('mapping.fullCoverage')}</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-amber-600 dark:text-amber-400">{stats.partialCoverage}</span>
 <span className="text-muted-foreground">{t('mapping.partialCoverage')}</span>
 </div>
 <div className="flex items-center gap-2">
 <span className="font-bold text-muted-foreground">{stats.unmapped}</span>
 <span className="text-muted-foreground">{t('mapping.unmapped')}</span>
 </div>
 </div>
 );
};
...
export const MappingMatrix: React.FC<MappingMatrixProps> = ({
 ...
 <div className="overflow-x-auto rounded-3xl border border-border/40 bg-card/50 backdrop-blur-sm">
 ...
 {frameworks.map((framework) => (
  ...
  className="px-3 py-3 text-center text-sm font-semibold text-foreground min-w-[100px]"
  >
 ...
 >
  ...
  <div className="text-sm text-foreground group-hover:text-primary dark:group-hover:text-primary/70 transition-colors line-clamp-1">
  {control.controlName}
  </div>
  </button>
  </td>
