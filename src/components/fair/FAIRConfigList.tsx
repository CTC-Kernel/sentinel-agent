...
const getComplexityBadge = (level: FAIRModelConfig['complexityLevel']) => {
 ...
 default:
 return { label: level, color: 'bg-muted text-foreground' };
 }
};
...
const ConfigCard: React.FC<ConfigCardProps> = ({
 ...
 hasSimulation && riskLevel ? riskLevel.bgColor : 'bg-muted'
 )}
 >
 <Calculator
 className={cn(
 'h-5 w-5',
 hasSimulation && riskLevel ? riskLevel.color : 'text-muted-foreground'
 )}
 />
 </div>
