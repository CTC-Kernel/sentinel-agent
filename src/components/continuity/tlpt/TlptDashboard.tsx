...
export const TlptDashboard: React.FC<Props> = ({ campaigns, loading, onAdd, onUpdate, onDelete, canEdit }) => {
 ...
 {[1, 2, 3].map((i) => (
 <div key={i || 'unknown'} className="h-48 bg-muted rounded-3xl animate-pulse" />
 ))}
 </div>
 );
 ...
 <div>
  <h3 className="text-lg font-semibold text-foreground">Campagnes de Tests de Résilience</h3>
  <p className="text-sm text-muted-foreground">Gestion des tests d'intrusion avancés (TLPT) et Red Teaming.</p>
 </div>
 {canEdit && (
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
  ...
  className="bg-card rounded-3xl p-5 border border-border/40 shadow-sm hover:shadow-md transition-shadow cursor-pointer group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
  ...
  <div className="flex items-center gap-2">
   ...
   <span>
   {campaign.startDate ? new Date(((campaign.startDate as unknown) as { seconds: number }).seconds ? ((campaign.startDate as unknown) as { seconds: number }).seconds * 1000 : campaign.startDate as Date | string | number).toLocaleDateString('fr-FR') : 'N/A'}
   </span>
  </div>
  </div>
  </div>
