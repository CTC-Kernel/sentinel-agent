...
export const ICTProviderList: React.FC<ICTProviderListProps> = ({
 ...
 <div className="flex flex-col">
  <span className="font-bold text-foreground truncate max-w-[250px]" title={row.original.name}>
  {row.original.name}
  </span>
  {row.original.services && row.original.services.length > 0 && (
  <span className="text-xs text-muted-foreground mt-0.5">
  {t('dora.provider.servicesCount', { defaultValue: '{{count}} service(s)', count: row.original.services.length })}
  </span>
  )}
 </div>
 )
 ...
 return (
  ...
  <RowActionsMenu
  items={menuItems}
  aria-label={t('dora.provider.actionsFor', { defaultValue: 'Actions pour {{name}}', name: row.original.name })}
  />
  </div>
 );
 },
 ...
 {[1, 2, 3].map(i => (
  <div key={i || 'unknown'} className="h-16 bg-muted animate-pulse rounded-3xl" />
 ))}
 </div>
 );
 }
