...
const markdownComponents: Components = {
 ...
 h2: ({ children }) => <h2 className="text-base font-semibold text-foreground /40 mt-4 mb-2">{children}</h2>,
 h3: ({ children }) => <h3 className="text-sm font-semibold text-foreground mt-3 mb-1.5">{children}</h3>,
 h4: ({ children }) => <h4 className="text-sm font-medium text-foreground mt-3 mb-1.5">{children}</h4>,
 ...
 blockquote: ({ children }) => (
 <blockquote className="border-l-4 border-primary/30 dark:border-primary/90 pl-4 py-1.5 my-3 italic text-muted-foreground bg-muted/30 rounded-r-lg">
 {children}
 </blockquote>
 ),

 // Lists
 ul: ({ children }) => <ul className="list-disc list-outside ml-4 space-y-1 mb-3 text-foreground pointer-events-none">{children}</ul>,
 ol: ({ children }) => <ol className="list-decimal list-outside ml-4 space-y-1 mb-3 text-foreground text-muted-foreground">{children}</ol>,
 li: ({ children }) => <li className="pl-1 leading-relaxed text-foreground text-muted-foreground">{children}</li>,
 ...
 td: ({ children }) => <td className="px-3 py-2 text-foreground whitespace-nowrap">{children}</td>,
 ...
 className="text-primary hover:text-primary underline underline-offset-2 transition-colors font-medium"
 >
 {children}
 </a>
 ),
 ...
 // Horizontal Rule
 hr: () => <hr className="my-6 border-border/40/50" />,
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, onCopy, copiedId, onUpgrade }) => {
 ...
 message.role === 'user'
  ...
  : "bg-primary/15 dark:bg-primary border-primary/30 dark:border-primary/40"
 )}>
 {message.role === 'user'
 ...
 <div className={cn(
  ...
  {!message.isError && (
  <div className="mt-3 pt-3 border-t border-border/40/50 flex gap-2">
   <button
   type="button"
  ...
  <div className="mt-4 pt-3 border-t border-error/20 dark:border-error/30">
   ...
   type="button"
   onClick={onUpgrade}
   className="w-full py-2 bg-gradient-to-r from-primary to-violet-600 hover:from-primary hover:to-violet-500 text-white rounded-lg text-xs font-bold shadow-lg shadow-primary/20 transition-all active:scale-95 flex items-center justify-center gap-2"
