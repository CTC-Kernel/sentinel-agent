...
export const SentinelAssistant: React.FC = () => {
 ...
 if (greeting === 'morning') return t('auth.greetingMorning', { defaultValue: 'Bonjour' });
 if (greeting === 'afternoon') return t('auth.greetingAfternoon', { defaultValue: 'Bon apres-midi' });
 return t('auth.greetingEvening', { defaultValue: 'Bonsoir' });
 };
 ...
 <div className="absolute -top-24 -right-24 w-64 h-64 bg-primary/20 dark:bg-primary/60/15 rounded-full blur-[80px] pointer-events-none group-hover:bg-primary/30/40 dark:group-hover:bg-primary/60/25 transition-colors duration-700"></div>
 ...
 <div className="relative h-[340px]">
  ...
  <h3 className="text-4xl font-extrabold text-foreground leading-tight">
   {getGreetingText()},<br />
   <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-600 dark:from-primary/80 dark:to-purple-400 animate-gradient-x bg-[length:200%_auto]">
   {t('auth.sentinelListening', { defaultValue: 'Je suis a votre ecoute.' })}
   </span>
  </h3>
  <p className="text-muted-foreground text-lg leading-relaxed font-normal">
   {t('auth.sentinelInstructions', { defaultValue: 'Passez la souris sur mon coeur ou commencez a taper pour discuter. Je peux vous informer sur notre securite, nos tarifs ou votre ROI.' })}
  </p>
  </div>

  <button
  onClick={() => setShowChat(true)}
  className="group flex items-center gap-3 px-6 py-3 rounded-3xl bg-muted/50 dark:bg-white/5 hover:bg-primary/10 dark:hover:bg-primary border border-border/40 hover:border-primary/30 dark:hover:border-primary/90 transition-all w-full text-left"
  >
  <div className="p-2 bg-primary/15 dark:bg-primary rounded-lg text-primary dark:text-primary/50 group-hover:scale-110 transition-transform">
   <Bot size={20} />
  </div>
  <div>
   <p className="text-sm font-bold text-foreground">{t('auth.askQuestion', { defaultValue: 'Poser une question' })}</p>
   <p className="text-xs text-muted-foreground">{t('auth.askQuestionExample', { defaultValue: '"Comment garantissez-vous la securite ?"' })}</p>
  </div>
  </button>
  </div>
  ) : (
