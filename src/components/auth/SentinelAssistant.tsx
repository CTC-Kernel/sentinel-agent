    ...
    return (
        <div className="hidden lg:flex flex-col justify-center w-full max-w-lg p-8 relative">
            ...
            <div className="relative z-10 glass-premium glass-noise rounded-[3rem] p-12 shadow-2xl overflow-hidden group hover:shadow-[0_0_50px_-12px_rgba(var(--brand-500),0.3)] hover:border-brand-500/30 transition-all duration-700 ease-out h-[600px] flex flex-col justify-between">
                ...
                <div className="flex items-center gap-4 mb-4" onMouseEnter={() => setShowChat(true)}>
                    ...
                    <div className="z-10">
                        <h2 className="text-2xl font-bold text-foreground tracking-tight flex items-center gap-2">
                            Sentinel-Core AI
                            <span className="px-2 py-0.5 rounded-full bg-primary/20 text-brand-600 dark:text-brand-300 text-[10px] border border-primary/30">BETA</span>
                        </h2>
                    ...
                    {!showChat ? (
                        <div className="animate-fade-in flex flex-col justify-center h-full space-y-6">
                            ...
                            <button
                                ...
                                className="group flex items-center gap-3 px-6 py-3 rounded-xl bg-muted/30 hover:bg-primary/5 border border-muted transition-all w-full text-left"
                            >
                                <div className="p-2 bg-primary/20 rounded-lg text-brand-600 dark:text-brand-300 group-hover:scale-110 transition-transform">
                                    <Bot size={20} />
                                </div>
                                <div>
                                    <p className="text-sm font-bold text-foreground">Poser une question</p>
                ...
                <div className="mt-10 pt-6 border-t border-muted flex justify-between items-center opacity-70 hover:opacity-100 transition-opacity">
                    <div className="flex items-center gap-2" title="Hébergement certifié SecNumCloud">
                        <Globe className="w-4 h-4 text-primary" />
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">SecNumCloud Ready</span>
                    </div>
                    <div className="flex items-center gap-2" title="Chiffrement de bout en bout">
                        <Lock className="w-4 h-4 text-success" />
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">End-To-End Encrypted</span>
                    </div>
                </div>
