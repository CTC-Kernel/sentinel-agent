...
export const RiskIntelCard: React.FC<RiskIntelCardProps> = ({ risks }) => {
    ...
    return (
        <PremiumCard
            glass
            className="mb-8 overflow-hidden relative border-t border-white/20 p-0 rounded-4xl"
            hover={false}
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 p-6 lg:p-8 relative z-decorator">
                ...
                <div className="lg:col-span-4 flex flex-col justify-between gap-6 border-r border-border/40 dark:border-white/5 pr-0 lg:pr-8">
                    <div>
                        ...
                        <p className="text-xs font-medium text-muted-foreground mt-1.5 opacity-70">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <motion.div
                            ...
                            className="bg-card p-4 rounded-3xl border border-white/40 dark:border-white/5 shadow-sm"
                        >
                            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Total Risques</p>
                        ...
                        >
                            <div className="flex items-center justify-between mb-1">
                                <p className={cn("text-xs font-bold uppercase tracking-wider", metrics.critical > 0 ? "text-error-text dark:text-error-text" : "text-success-text dark:text-success-text")}>
                                    Élevés+
                                </p>
                    ...
                    <div className="flex items-center gap-3 text-xs p-3.5 bg-muted/30 dark:bg-white/5 rounded-3xl border border-border/40 dark:border-white/5">
                        <div className={cn("p-1.5 rounded-xl", metrics.aboveAppetite > 0 ? "bg-warning/20" : "bg-muted")}>
                            <AlertTriangle className={cn("w-4 h-4", metrics.aboveAppetite > 0 ? "text-warning" : "text-muted-foreground")} />
                        </div>
