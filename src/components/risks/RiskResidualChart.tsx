...
export const RiskResidualChart: React.FC<RiskResidualChartProps> = ({ risks }) => {
 const uid = useId();
 const { t } = useLocale();
 const inherentGradientId = `inherentGradient-${uid}`;
 const residualGradientId = `residualGradient-${uid}`;

 // Aggregation: Avg Inherent vs Avg Residual by Category
 const data = React.useMemo(() => {
 const defaultCategory = t('risks.charts.defaultCategory', { defaultValue: 'Majeur' });
 const categories = Array.from(new Set(risks.map(r => r.category || defaultCategory)));
 return categories.map(cat => {
 const catRisks = risks.filter(r => (r.category || defaultCategory) === cat);
 const total = catRisks.length;
 const avgInherent = catRisks.reduce((sum, r) => sum + r.score, 0) / total;
 const avgResidual = catRisks.reduce((sum, r) => sum + (r.residualScore || r.score), 0) / total;
 ...
 }).sort((a, b) => b.inherent - a.inherent).slice(0, 6);
 }, [risks, t]);

 const hasData = data.length > 0 && data.some(d => d.inherent > 0 || d.residual > 0);

 if (!hasData) {
 return <EmptyChartState variant="bar" message={t('risks.charts.noAnalysisData', { defaultValue: "Pas de données d'analyse" })} description={t('risks.charts.noAnalysisDescription', { defaultValue: 'Déclarez des risques et leurs scores pour voir la réduction.' })} />;
 }
 ...
 <BarChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
  <defs>
  <linearGradient id={inherentGradientId} x1="0" y1="0" x2="0" y2="1">
  ...
  <linearGradient id={residualGradientId} x1="0" y1="0" x2="0" y2="1">
  ...
  <Bar
  dataKey="inherent"
  name={t('risks.charts.inherent', { defaultValue: 'Inhérent' })}
  fill={`url(#${inherentGradientId})`}
  radius={[6, 6, 0, 0]}
  maxBarSize={50}
  ...
  <Bar
  dataKey="residual"
  name={t('risks.charts.residual', { defaultValue: 'Résiduel' })}
