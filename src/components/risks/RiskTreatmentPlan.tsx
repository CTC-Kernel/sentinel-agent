...
export const RiskTreatmentPlan: React.FC<RiskTreatmentPlanProps> = ({ risk, onUpdate, onRiskUpdate, users, controls = [], onDirtyChange }) => {
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  ...
  <label htmlFor="risk-strategy" className="text-xs font-bold uppercase tracking-wider text-foreground ml-1">Stratégie</label>
  ...
  <AlertTriangle className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none group-hover:text-primary transition-colors" />
  </div>
  {showAcceptWarning && (
  ...
  <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none group-hover:text-primary transition-colors" />
  </div>
  </div>
  ...
  <div className="relative group">
  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-bold group-focus-within:text-primary transition-colors">€</span>
  <input
 ...
 <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 shadow-sm space-y-4">
  ...
  <p className="text-sm text-foreground text-muted-foreground">{measure}</p>
 ...
 <div className="glass-premium p-4 sm:p-6 rounded-4xl border border-border/40 shadow-sm space-y-4">
  ...
  <span className="text-xs font-medium text-muted-foreground">Couverture:</span>
  ...
  return (
   <div key={ctrl.id || 'unknown'} className="flex items-center justify-between p-3 bg-card rounded-3xl border border-border/40 shadow-sm group hover:shadow-md transition-shadow">
   <div className="flex items-center gap-3">
   <Shield className={`h-4 w-4 ${isImplemented ? 'text-success-text' : isPartial ? 'text-warning-text' : 'text-muted-foreground'}`} />
   <div>
   <p className="text-sm font-bold text-foreground">{ctrl.code} - {ctrl.name}</p>
   <div className="flex items-center gap-2 mt-0.5">
    <span className="text-xs text-muted-foreground">{ctrl.framework || 'Contrôle'}</span>
    <Badge
  ...
  <div className="max-h-48 overflow-y-auto space-y-1 rounded-3xl border border-border/40 bg-white/30 dark:bg-black/10 p-2">
  ...
  filteredControls.slice(0, 20).map(ctrl => {
   ...
   <Plus className="h-4 w-4 text-primary opacity-0 group-hover:opacity-70 transition-opacity" />
   </div>
   </button>
   );
  })
