...
export const RisksToolbar: React.FC<RisksToolbarProps> = ({
 ...
 <>
  ...
  className={showAdvancedSearch ? 'bg-primary/10 text-primary border-primary/20' : ''}
  >
  ...
  {({ open }) => (
  <>
  <Menu.Button ref={toolsMenuButtonRef} as={Button} variant="outline" size="icon" aria-label={t('risks.moreActions') || 'Plus d\'actions'}>
  ...
  <div className="p-1">
   <Menu.Item>
   {({ active }) => (
   <button onClick={handleCommonExport} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
   ...
   <Menu.Item>
   {({ active }) => (
   <button onClick={onExportExcel} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
   ...
   <Menu.Item>
   {({ active }) => (
   <button onClick={() => exportCSV(filteredRisks)} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
   ...
   <Menu.Item>
   {({ active }) => (
   <button onClick={() => ObsidianService.exportRisksToObsidian(filteredRisks)} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
  ...
  {canEdit && (
   ...
   <Menu.Item>
   {({ active }) => (
   <button onClick={() => setImportModalOpen(true)} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
   ...
   <Menu.Item>
   {({ active }) => (
   <button onClick={() => setIsTemplateModalOpen(true)} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
 ...
 <>
  ...
  <button
  onClick={onClearAll}
  className="text-xs font-bold uppercase tracking-wider text-muted-foreground hover:text-red-500 transition-colors ml-auto px-2 py-1"
  >
