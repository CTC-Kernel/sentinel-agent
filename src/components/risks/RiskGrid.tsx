...
export const RiskGrid: React.FC<RiskGridProps> = ({
 ...
 return { status: 'ok', days: diffDays, label: `${diffDays} j`, color: 'text-muted-foreground bg-muted border-border/40 shadow-sm shadow-black/5' };
 };
 ...
 return (
  ...
  <CustomTooltip content="Modifier">
   ...
   size="sm"
   variant="secondary"
   className="h-8 w-8 p-0 bg-white/90 dark:bg-white/10 shadow-sm backdrop-blur-sm hover:text-primary transition-all hover:scale-110 rounded-3xl"
  ...
  <CustomTooltip content={deletingIds.has(risk.id) ? "Suppression..." : "Supprimer"}>
   ...
   size="sm"
   variant="secondary"
   className="h-8 w-8 p-0 bg-white/90 dark:bg-white/10 shadow-sm backdrop-blur-sm hover:text-error-text transition-all hover:scale-110 rounded-3xl"
  ...
  <div className="flex items-center mb-3">
   <div className="p-1.5 rounded-lg bg-muted text-foreground mr-2.5"><Server className="w-3.5 h-3.5" /></div>
   <span className="text-xs font-bold text-foreground uppercase tracking-wide truncate">{getAssetName(risk.assetId)}</span>
  </div>
  <h4 className="text-lg font-bold text-foreground leading-snug mb-2 line-clamp-2">
   <TextHighlight text={risk.threat} query={searchQuery || ''} />
  </h4>
  <div className="text-sm text-foreground bg-secondary/50 p-3 rounded-2xl inline-block w-full border border-border/40">
   <span className="font-bold text-xs uppercase text-foreground block mb-1">{t('common.vulnerability')}</span>
  ...
  <div className="flex items-center justify-between">
   ...
   <span className="text-xs font-bold text-foreground bg-secondary/50 px-2 py-1 rounded-lg border border-border/40">{risk.strategy}</span>
   {(() => {
