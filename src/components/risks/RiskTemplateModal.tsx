...
export const RiskTemplateModal: React.FC<RiskTemplateModalProps> = ({ isOpen, onClose, onSelectTemplate, users, isLoading = false }) => {
 ...
 case 'Cloud': return 'from-info-text to-primary';
 default: return 'from-muted/500 to-slate-600';
 }
 };

 return (
 <Transition.Root show={isOpen} as={React.Fragment}>
 <Dialog as="div" className="relative z-max" onClose={onClose}>
 <Transition.Child
  as={React.Fragment}
 ...
 <div className="fixed inset-0 z-decorator overflow-y-auto">
  ...
  >
  <Dialog.Panel className="bg-card rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-border/40 text-left align-middle transition-all transform">
  {/* Header */}
  <div className="flex items-center justify-between p-6 border-b border-border/40">
   <div>
   <Dialog.Title as="h2" className="text-2xl font-bold text-foreground">
  ...
  <div className="overflow-y-auto max-h-[calc(90vh-200px)]">
   ...
   <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
    ...
    <div key={risk.threat || 'unknown'} className="flex items-center justify-between p-2 bg-card rounded-lg text-xs border border-border/40 dark:border-white/5">
    <span className="font-medium text-foreground flex-1 truncate">
    {risk.threat}
    </span>
    <div className="flex items-center gap-2">
    {/* <span className="text-muted-foreground">{risk.category || 'N/A'}</span> */}
