...
export const IntegrationCard: React.FC<IntegrationCardProps> = ({ provider, onConnect, onDisconnect, isConnecting }) => {
    ...
    return (
        <div className={`group relative p-6 rounded-3xl border transition-all duration-300 ${isConnected
            ...
            : 'glass-panel hover:shadow-xl hover:border-brand-500/30 dark:hover:border-white/20'
            }`}>
            {/* Inner Glow Effect */}
            ...
            <div className="flex items-start justify-between mb-4 relative z-10">
                <div className={`p-3.5 rounded-2xl transition-colors duration-300 ${isConnected
                    ...
                    : 'bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 group-hover:bg-brand-50 dark:group-hover:bg-brand-500/20 group-hover:text-brand-600 dark:group-hover:text-brand-400'
                    }`}>
            ...
            <div className="flex items-center justify-between mt-auto relative z-10">
                <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${isConnected
