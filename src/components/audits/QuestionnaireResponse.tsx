    ...
    const renderEvidenceSection = (questionId: string) => {
        ...
        return (
            <div className="mt-3">
                ...
                {isOpen && (
                    <div className="mt-2 p-3 bg-slate-50 dark:bg-white/5 rounded-3xl border border-dashed border-border/40 dark:border-border/40">
    ...
    return (
        <div className="fixed inset-0 bg-[var(--overlay-bg)] backdrop-blur-[var(--overlay-blur)] z-modal flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                ...
                {/* Footer */}
                <div className="p-6 border-t border-border/40 dark:border-border/40 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                    <button
                        aria-label="Fermer la vue questionnaire"
                        onClick={onClose}
                        className="px-4 py-2 text-slate-600 dark:text-slate-300 font-bold hover:bg-slate-200 dark:hover:bg-white/5 rounded-3xl transition-colors"
                    >
                        Fermer
                    </button>
                    {!readOnly && status !== 'Submitted' && (
                        <>
                            <button
                                ...
                                disabled={saving}
                                className="px-4 py-2 text-brand-600 font-bold hover:bg-brand-50 dark:hover:bg-white/5 rounded-3xl transition-colors"
                            >
