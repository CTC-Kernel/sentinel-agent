...
export const AuditFindings: React.FC<AuditFindingsProps> = ({
 ...
 <form onSubmit={findingForm.handleSubmit(onSubmit)} className="glass-premium p-6 rounded-3xl border border-border/40 shadow-sm">
  ...
  <Plus className="h-4 w-4 text-primary" /> {t('audits.findingsSection.newFinding')}
  </h3>
 ...
 <h3 className="text-lg font-bold text-foreground">{t('audits.findingsSection.listTitle', { count: findings.length })}</h3>
 {findings.length === 0 ? (
  <EmptyState
  icon={AlertOctagon}
  ...
  {canEdit && (
   ...
   <label htmlFor={`file-upload-${f.id}`} className={`cursor-pointer p-2 transition-all rounded-3xl hover:bg-indigo-500/10 ${uploadingFindingId === f.id ? 'text-indigo-500 animate-pulse' : 'text-muted-foreground hover:text-indigo-500'}`}>
   ...
   <button onClick={() => onDeleteFinding(f.id)} aria-label={t('audits.findingsSection.delete')} className="text-muted-foreground hover:text-red-500 p-2 rounded-3xl hover:bg-red-50 dark:hover:bg-red-900/30 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
