...
export const QuestionnaireList: React.FC<QuestionnaireListProps> = ({ auditId, organizationId, canEdit }) => {
 ...
 <h3 className="text-lg font-bold text-foreground">{t('audits.questionnaire.title', { defaultValue: "Questionnaires d'Audit" })}</h3>
 {canEdit && (
  <button
 ...
 {questionnaires.length === 0 && (
  <div className="col-span-full text-center py-12 text-muted-foreground italic bg-muted/50 dark:bg-white/5 rounded-2xl border border-dashed border-border/40">
  {t('audits.questionnaire.empty', { defaultValue: 'Aucun questionnaire créé pour cet audit.' })}
  </div>
 )}
 {questionnaires.map(q => (
  <div key={q.id || 'unknown'} className="bg-card p-6 rounded-2xl border border-border/40 dark:border-white/5 shadow-sm hover:shadow-md transition-all group relative">
  <div className="flex justify-between items-start mb-4">
  <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 rounded-3xl">
  <FileText className="w-6 h-6" />
  </div>
  <div className="flex gap-2">
  <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wide ${q.status === 'Published' ? 'bg-emerald-50 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>
   {q.status === 'Published' ? t('common.published', { defaultValue: 'Publié' }) : t('common.draft', { defaultValue: 'Brouillon' })}
  </span>
  </div>
  </div>

  <h4 className="text-lg font-bold text-foreground mb-2">{q.title}</h4>
