...
export const AssignPartnerDrawer: React.FC<AssignPartnerDrawerProps> = ({ isOpen, onClose, auditId, auditName, onAssigned }) => {
 ...
 {loading ? (
  ...
  <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-muted/30 rounded-3xl border border-dashed border-border/40">
  <Building2 className="w-10 h-10 text-muted-foreground mb-3" />
  <p className="text-muted-foreground font-medium">{t('assignPartner.noPartnerFound', { defaultValue: 'Aucun partenaire trouvé' })}</p>
  {partners.length === 0 && (
  ...
  disabled={!!assigning}
  className="w-full flex items-center justify-between p-4 rounded-3xl border border-border/40 dark:border-white/5 hover:border-primary/60 dark:hover:border-primary/60 hover:bg-muted/50 transition-all text-left group"
  >
  ...
  {assigning === partner.id ? (
   <Loader2 className="w-5 h-5 animate-spin text-primary ml-3 shrink-0" />
  ) : (
   <span className="ml-3 shrink-0 text-xs font-semibold bg-muted text-muted-foreground px-3 py-1.5 rounded-lg group-hover:bg-primary/10 group-hover:text-primary dark:group-hover:bg-primary dark:group-hover:text-primary/70 transition-colors">
