...
export const EvidenceRequestItem: React.FC<EvidenceRequestItemProps> = React.memo(({
    ...
    return (
        <div key={req.id || 'unknown'} className="bg-white dark:bg-slate-800 border border-border/40 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm transition-all hover:shadow-md hover:border-brand-200 dark:hover:border-brand-600 group">
            <div
                className="p-3.5 flex items-center justify-between cursor-pointer hover:bg-slate-50/50 dark:hover:bg-white/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-3xl"
            ...
            >
                <div className="flex items-center gap-4 min-w-0">
                    <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${req.status === 'Accepted' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20' : 'bg-slate-100 dark:bg-slate-800 text-muted-foreground dark:bg-slate-800'}`}>
            ...
            {isExpanded && (
                <div className="p-6 border-t border-border/40 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
                    <div className="mb-6">
                        <h5 className="text-xs font-bold uppercase text-muted-foreground tracking-wider mb-2">Description</h5>
                        <p className="text-sm text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 p-4 rounded-3xl border border-border/40 dark:border-white/5">
                            {req.description}
                        </p>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        {req.documentIds && req.documentIds.length > 0 ? (
                                ...
                                {req.documentIds.map(docId => {
                                    ...
                                    return (
                                        <div key={docId || 'unknown'} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-3xl border border-border/40 dark:border-border/40 text-sm hover:border-brand-300 transition-colors group">
                                            ...
                                            <a href={docObj.url} target="_blank" rel="noreferrer" className="text-brand-600 hover:text-brand-700 text-xs font-bold px-3 py-1 bg-brand-50 dark:bg-brand-800 rounded-lg opacity-0 group-hover:opacity-70 transition-opacity focus:opacity-70 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">
                                                Voir
                                            </a>
                                        </div>
                                    );
                            ...
                            <div className="text-center py-6 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-border/40 dark:border-border/40">
                    ...
                    {/* Actions */}
                    <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-border/40 dark:border-border/40">
