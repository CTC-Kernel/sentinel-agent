...
const typeBgColors: Record<string, string> = {
 'Interne': 'bg-primary/10 border-primary/30 dark:border-primary/90',
...
export const AuditMethodsWorkshops: React.FC<AuditMethodsWorkshopsProps> = ({
 ...
 {t('audits.methods.title', { defaultValue: "Méthodes & Ateliers d'Audit" })}
 </h2>
 <p className="text-sm text-muted-foreground mt-1">
 {t('audits.methods.subtitle', { defaultValue: 'Méthodologies structurées pour conduire vos audits de sécurité' })}
 </p>
 </div>
 {activeWorkshop && (
 <Button variant="outline" size="sm" onClick={handleResetWorkshop} className="gap-2">
 <RotateCcw className="w-4 h-4" />
 {t('audits.methods.resetWorkshop', { defaultValue: "Réinitialiser l'atelier" })}
 </Button>
 )}
 </div>
 ...
 <div className="flex items-start justify-between mb-4">
  ...
  {isActive && (
  <Badge status="success" size="sm">{t('audits.methods.inProgress', { defaultValue: 'En cours' })}</Badge>
  )}
 </div>
 ...
 {isActive && (
  ...
  <span className="text-xs font-medium text-muted-foreground">{t('audits.methods.progress', { defaultValue: 'Progression' })}</span>
  <span className="text-xs font-bold text-foreground">{workshopProgress}%</span>
  </div>
 ...
 {!activeWorkshop || activeWorkshop.templateId !== selectedTemplate.id ? (
  ...
  {t('audits.methods.startWorkshop', { defaultValue: "Démarrer l'atelier" })}
  </Button>
 ) : (
  <Badge status="success" className="text-sm px-4 py-2">
  <CheckCircle className="w-4 h-4 mr-2" />
  {t('audits.methods.workshopInProgress', { defaultValue: 'Atelier en cours' })}
  </Badge>
 )}
 </div>
 ...
 <div className="p-4 rounded-3xl bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-200 dark:border-indigo-800">
  ...
  {t('audits.methods.bestPractices', { defaultValue: 'Bonnes pratiques' })}
  </h4>
  <ul className="space-y-2">
  {selectedTemplate.bestPractices.map((practice, i) => (
  <li key={i || 'unknown'} className="flex items-start gap-2 text-sm text-muted-foreground">
 ...
 <div className="p-4 rounded-3xl bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800">
  ...
  {t('audits.methods.expectedDeliverables', { defaultValue: 'Livrables attendus' })}
  </h4>
  <ul className="space-y-2">
 ...
 {t('audits.methods.auditPhases', { defaultValue: "Phases de l'audit" })}
 </h4>
 ...
 return (
  ...
  <div>
  <h5 className="font-bold text-foreground">{phase.name}</h5>
  ...
  {completedTasks}/{totalTasks} {t('audits.methods.tasks', { defaultValue: 'tâches' })}
  </span>
  ...
  >
   ...
   {task.isRequired && (
   <Badge status="warning" size="sm">{t('audits.methods.required', { defaultValue: 'Requis' })}</Badge>
   )}
   </div>
  ...
  <h6 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">
   {t('audits.methods.phaseDeliverables', { defaultValue: 'Livrables de cette phase' })}
  </h6>
 ...
 {t('audits.methods.selectMethod', { defaultValue: "Sélectionnez une méthode d'audit" })}
 </h3>
 <p className="text-muted-foreground">
 {t('audits.methods.selectMethodDesc', { defaultValue: "Choisissez un type d'audit ci-dessus pour voir la méthodologie détaillée et démarrer un atelier interactif." })}
 </p>
 </div>
 </PremiumCard>
 )}
