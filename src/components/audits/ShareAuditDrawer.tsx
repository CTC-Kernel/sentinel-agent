...
export const ShareAuditDrawer: React.FC<ShareAuditDrawerProps> = ({ isOpen, onClose, auditId, auditName }) => {
 ...
 {!generatedLink ? (
  ...
  <div className="bg-primary/10 dark:bg-primary p-4 rounded-3xl border border-primary/20 dark:border-primary/90 flex items-start gap-4">
  <div className="bg-primary/15 dark:bg-primary p-2 rounded-lg text-primary dark:text-primary/50">
  ...
  <div className="absolute -inset-1 bg-gradient-to-r from-primary to-purple-600 rounded-3xl opacity-20 group-hover:opacity-40 blur transition duration-200"></div>
  <div className="relative flex items-center bg-card border border-border/40 rounded-3xl p-1 pr-1.5 shadow-sm">
  <div className="pl-4 py-3 flex-1 overflow-hidden">
   <p className="text-sm font-mono text-muted-foreground truncate select-all">
   {generatedLink}
   </p>
  </div>
  <Button size="sm" onClick={copyLink} className="shrink-0 bg-card text-white hover:bg-muted dark:bg-primary dark:text-primary-foreground gap-2">
