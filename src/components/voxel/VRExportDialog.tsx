...
const PlatformCard: React.FC<PlatformCardProps> = ({ platform, isSelected, onSelect }) => {
 ...
 {/* Icon */}
 <div className={`mb-3 ${isSelected ? 'text-blue-400' : 'text-muted-foreground'}`}>
 <Icon />
 </div>

 {/* Name */}
 <h3 className={`font-semibold text-sm ${isSelected ? 'text-white' : 'text-muted-foreground/60'}`}>
 {settings.name}
 </h3>
...
export const VRExportDialog: React.FC<VRExportDialogProps> = ({
 ...
 {/* Platform Selection */}
 <div>
 <h3 className="text-sm font-medium text-muted-foreground mb-3 block">Target Platform</h3>
 ...
 {/* Filename */}
 <div>
 <label htmlFor="filename-input" className="text-sm font-medium text-muted-foreground mb-2 block">Filename</label>
 <input
 id="filename-input"
 type="text"
 value={filename}
 ...
 <span className="float-right text-muted-foreground uppercase">
