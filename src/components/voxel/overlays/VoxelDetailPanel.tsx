...
const DetailRow: React.FC<DetailRowProps> = ({ label, value, icon }) => (
 ...
 <span className="text-sm text-muted-foreground/60 font-medium">{value}</span>
 </div>
);
...
const LinkedEntityItem: React.FC<LinkedEntityItemProps> = ({ node, onClick }) => {
 ...
 />
 <ChevronRight className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-70 transition-opacity" />
 </motion.button>
 );
};
