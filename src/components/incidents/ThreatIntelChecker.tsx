...
export const ThreatIntelChecker: React.FC = () => {
 ...
 return (
 <div className="bg-card/50 p-6 rounded-2xl border border-border/40 shadow-sm">
 <h3 className="text-sm font-bold text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
 <ShieldAlert className="h-4 w-4" />
 {t('incidents.threatIntel.title', { defaultValue: 'Threat Intel' })}
 </h3>
 <div className="space-y-3">
 <FloatingLabelInput
  label={t('incidents.threatIntel.checkUrlLabel', { defaultValue: 'Vérifier une URL / IP' })}
  value={urlToCheck}
 ...
 <Button
  ...
  variant="secondary"
 >
  {t('incidents.threatIntel.checkReputation', { defaultValue: 'Vérifier la réputation' })}
 </Button>
 ...
 {urlReputationResult && (
  ...
  <>
  <ShieldAlert className="h-4 w-4" />
  {t('incidents.threatIntel.safe', { defaultValue: 'URL/IP saine' })}
  </>
  ) : (
  <>
  <ShieldAlert className="h-4 w-4" />
  {t('incidents.threatIntel.threatDetected', { defaultValue: 'Menace détectée' })}: {urlReputationResult.threatType}
  </>
  )}
  </div>
 )}
