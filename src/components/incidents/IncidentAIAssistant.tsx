...
export const IncidentAIAssistant: React.FC<IncidentAIAssistantProps> = ({ incident }) => {
    ...
    const handleAnalyzeIncident = async () => {
        ...
        try {
            ...
            setAnalysisError('L\'analyse IA a \u00e9chou\u00e9. V\u00e9rifiez votre connexion et r\u00e9essayez.');
        } finally {
            setAnalyzing(false);
        }
    };
            ...
            {!aiAnalysis ? (
                <div className="bg-slate-50 dark:bg-white/5 p-8 rounded-3xl border border-dashed border-border/40 dark:border-border/40 text-center">
                    ...
                    <p className="text-sm text-slate-600 dark:text-muted-foreground mb-6 max-w-md mx-auto">
                        L'IA peut analyser les d&eacute;tails de l'incident pour identifier la cause racine probable et sugg&eacute;rer des mesures correctives.
                    </p>
                    {analysisError && (
                        <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl text-left">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-bold text-red-700 dark:text-red-400">Erreur d'analyse</span>
                            </div>
