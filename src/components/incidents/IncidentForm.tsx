...
export const IncidentForm: React.FC<IncidentFormProps> = ({
 ...
 <div className="bg-[var(--glass-bg)] backdrop-blur-xl p-4 sm:p-6 rounded-xl border border-error/30 space-y-4 relative overflow-hidden group">
  ...
  <Controller
   ...
   render={({ field }) => (
   <CustomSelect
   label={t('incidents.form.notificationStatus', { defaultValue: 'Statut Notification' })}
   value={field.value || ''}
 ...
 <div className="relative">
  ...
  render={({ field }) => (
  <RichTextEditor
  label={t('incidents.form.description', { defaultValue: 'Description détaillée' })}
  value={field.value || ''}
  onChange={field.onChange}
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.category', { defaultValue: 'Catégorie (Playbook)' })}
  value={field.value || ''}
  onChange={field.onChange}
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.severity', { defaultValue: 'Sévérité' })}
  value={field.value || ''}
  onChange={field.onChange}
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.status', { defaultValue: 'Statut' })}
  value={field.value || ''}
  onChange={field.onChange}
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.affectedAsset', { defaultValue: 'Actif Impacté' })}
  value={field.value || ''}
  onChange={field.onChange}
  options={[
   { value: '', label: t('incidents.form.noneUnknown', { defaultValue: 'Aucun / Inconnu' }) },
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.affectedProcess', { defaultValue: 'Processus Impacté' })}
  value={field.value || ''}
  onChange={field.onChange}
  options={[
   { value: '', label: t('incidents.form.noneUnknown', { defaultValue: 'Aucun / Inconnu' }) },
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.relatedRisk', { defaultValue: 'Lier à un Risque Identifié' })}
  value={field.value || ''}
  onChange={field.onChange}
  options={[
   { value: '', label: t('incidents.form.notLinked', { defaultValue: 'Non lié' }) },
  ...
  render={({ field }) => (
  <CustomSelect
  label={t('incidents.form.reporter', { defaultValue: 'Déclaré par' })}
  value={field.value || ''}
  onChange={field.onChange}
  options={[
   { value: '', label: t('common.select', { defaultValue: 'Sélectionner...' }) },
  ...
  <FloatingLabelInput
  label={t('incidents.form.estimatedCost', { defaultValue: 'Coût estimé (€)' })}
  type="number"
