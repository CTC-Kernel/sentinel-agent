...
function validateAgentMetrics(data: Record<string, unknown>): Record<string, unknown> {
    ...
    // Validate CPU - log if value exceeds 100%
...
/**
 * Subscribe to agent live data (processes, connections)
 */
export function subscribeToAgentLiveData<T>(
    organizationId: string,
    agentId: string,
    dataType: 'processes' | 'connections',
    onData: (data: T) => void,
    onError?: (error: Error) => void
): Unsubscribe {
    ...
    return onSnapshot(
        ...
        (snapshot: DocumentSnapshot) => {
            if (snapshot.exists()) {
                ...
                onData(data?.[dataType] as T);
            }
        },
        (error: FirestoreError) => {
            ErrorLogger.error(error, `AgentService.subscribeToAgentLiveData.${dataType}`, {
                component: 'AgentService',
                agentId,
                dataType
            });
...
export async function deleteAgent(
    ...
    try {
        const deleteAgentFn = httpsCallable<
            { organizationId: string; agentId: string; deleteResults: boolean; hideLabel?: boolean; },
            { success: boolean }
