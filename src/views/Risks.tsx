...
export const Risks: React.FC = () => {
    ...
    const handleDelete = useCallback(async (risk: { id: string, name: string }) => {
        if (!canEditResource(user as UserProfile, 'Risk')) {
            toast.error(t('errors.permissionDenied') || t('risks.errors.permissionDenied', { defaultValue: 'Permission denied' }));
            return;
        }
    ...
    const handleCommonExport = useCallback(async () => {
        ...
        try {
            await PdfService.generateRiskExecutiveReport(filteredRisks, {
                ...
                subtitle: t('risks.generatedBy', { defaultValue: 'Generated by {{name}} on {{date}}', name: user?.displayName || t('common.user', { defaultValue: 'User' }), date: new Date().toLocaleDateString() }),
    ...
    const handleExportExcel = useCallback(async () => {
        try {
            ...
            toast.success(t('risks.exportSuccess') || t('risks.excelExportSuccess', { defaultValue: 'Excel export successful' }));
        } catch (e) {
    ...
    const handleStartAiAnalysis = useCallback(async () => {
        ...
        try {
            ...
            toast.info(t('risks.analysisComplete'), typeof analysis === 'string' ? analysis : t('risks.analysisFinished', { defaultValue: 'Analysis complete' }));
        } catch (e) {
    ...
    const handleDuplicateRisk = useCallback(async (r: Risk) => {
        ...
        try {
            ...
            toast.success(t('risks.duplicateSuccess') || t('risks.duplicated', { defaultValue: 'Risk duplicated successfully' }));
        } catch {
            toast.error(t('risks.duplicateError') || t('risks.errors.duplicationFailed', { defaultValue: 'Error during duplication' }));
        } finally {
    ...
    const tabs = useMemo(() => [
        ...
        { id: 'context', label: t('risks.context') || t('risks.tabs.context', { defaultValue: 'Context' }), icon: Target },
        { id: 'ebios', label: t('risks.ebios') || t('risks.tabs.ebios', { defaultValue: 'EBIOS RM' }), icon: Scale },
        { id: 'list', label: t('risks.registry'), icon: List },
        { id: 'financial', label: t('risks.financial') || t('risks.tabs.financial', { defaultValue: 'Financial Risks' }), icon: Calculator },
            ...
            {showSaveViewModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-card rounded-2xl p-6 max-w-md mx-4 shadow-apple-xl border border-border/50">
                        <h3 className="text-lg font-semibold mb-4">{t('risks.saveViewTitle', { defaultValue: 'Save view' })}</h3>
                        <input
                            type="text"
                            value={viewName}
                            onChange={(e) => setViewName(e.target.value)}
                            placeholder={t('risks.viewNamePlaceholder', { defaultValue: 'View name...' })}
