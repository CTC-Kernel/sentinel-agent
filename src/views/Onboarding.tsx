...
export const Onboarding: React.FC = () => {
 ...
 return (
 <div className="min-h-screen py-10 flex items-center justify-center relative font-sans selection:bg-primary selection:text-white">
 ...
 <div className="glass-premium rounded-3xl p-10 md:p-12 shadow-2xl">
  ...
  <div className="flex items-center gap-4 w-full">
   ...
   <h2 className="text-lg font-bold text-foreground mb-1">{t('onboarding.actions.createOrg')}</h2>
   <p className="text-sm text-muted-foreground font-normal">{t('onboarding.actions.createDesc')}</p>
   </div>
  </div>
  </Button>
  ...
  <div className="flex items-center gap-4 w-full">
   ...
   <div>
   <h3 className="text-lg font-bold text-foreground mb-1">{t('onboarding.actions.joinOrg')}</h3>
   <p className="text-sm text-muted-foreground font-normal">{t('onboarding.actions.joinDesc')}</p>
   </div>
  </div>
  </Button>
  </div>
  ...
  <>
   ...
   isLoading={loading}
   size="sm"
   className="absolute right-2 top-2 px-4 py-1.5 bg-foreground text-background rounded-xl font-bold text-xs shadow-lg disabled:bg-muted disabled:text-muted-foreground h-auto"
   >
   ...
   <div>
    ...
    <p className="text-xs text-muted-foreground">{org.industry || 'Non spécifié'}</p>
   </div>
   <Button
    ...
    isLoading={loading}
    size="sm"
    className="px-4 py-2 bg-primary/10/20 text-primary rounded-xl text-sm font-bold hover:bg-primary/15 dark:hover:bg-primary transition-colors disabled:bg-muted disabled:text-muted-foreground h-auto"
   >
   ...
   <p className="text-center text-muted-foreground text-sm py-4">{t('onboarding.toasts.emptySearch') || "Aucune organisation trouvée."}</p>
   )}
   </div>
  </>
  ) : (
  ...
  <form onSubmit={form.handleSubmit(handleStep1)} className="space-y-6">
   ...
   className="w-4 h-4 text-primary border-border rounded focus-visible:ring-primary bg-white dark:bg-black/20 dark:border-white/10"
   />
   </div>
   </div>
   <label htmlFor="terms" className="text-sm text-muted-foreground">
   {t('onboarding.form.terms')} <Button variant="link" size="sm" aria-label="Lire les conditions générales" type="button" onClick={() => { setLegalTab('terms'); setShowLegalModal(true); }} className="text-primary font-bold hover:underline px-0 h-auto">{t('onboarding.form.conditions')}</Button>...
   </label>
  ...
  <div className="space-y-6">
   ...
   <div
    ...
    ? 'bg-primary/10/10 border-primary ring-1 ring-primary shadow-lg shadow-primary/25'
    : 'bg-white/50 dark:bg-white/5 border-border hover:border-primary/40 dark:hover:border-primary/80 hover:shadow-md'
    }`}
   >
    {/* Plan content */}
    <div className="flex justify-between items-start mb-3">
    <div>
    <h3 className={`font-bold text-lg ${isSelected ? 'text-primary dark:text-primary/70' : 'text-foreground'}`}>
    ...
    <span className={`block text-xl font-bold font-display tracking-tight ${isSelected ? 'text-primary dark:text-primary/70' : 'text-foreground'}`}>
    ...
    <li key={f || 'unknown'} className="flex items-center text-xs font-medium text-muted-foreground">
    <div className={`mr-2 p-0.5 rounded-full ${isSelected ? 'bg-primary/15/30 text-primary' : 'bg-muted text-muted-foreground'}`}>
  ...
  <div className="space-y-6 animate-fade-in">
   <div>
   <span className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4 ml-1 flex items-center gap-2">
   ...
   >
    ...
    {standards.includes(fw.name) && <Check className="h-5 w-5 text-primary" strokeWidth={3} />}
   </div>
   ))}
   </div>
   </div>
   ...
   <div className="pt-4 flex gap-3">
   <div className="pt-4 flex gap-3">
   <Button aria-label="Retour à l'étape 2" onClick={() => setStep(2)} variant="ghost" className="w-1/3 py-4 bg-muted text-muted-foreground font-bold rounded-2xl hover:bg-muted transition-colors h-auto">{t('onboarding.actions.back')}</Button>
   <Button aria-label="Valider le périmètre et continuer" onClick={handleStep3} disabled={loading} isLoading={loading} className="w-2/3 py-4 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-2xl shadow-lg shadow-primary/20 flex items-center justify-center group disabled:bg-muted disabled:text-muted-foreground h-auto">
   {!loading && <>{t('onboarding.actions.continue')} <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" strokeWidth={2.5} /></>}
   </Button>
   </div>
   </div>
  ...
  <div className="space-y-6 animate-fade-in">
   ...
   <Button aria-label="Inviter l'utilisateur" onClick={handleInviteUser} className="px-4 py-3 bg-foreground text-background rounded-2xl font-bold hover:opacity-90 transition-opacity h-[56px] flex items-center justify-center">
   ...
   <div key={userInvite.email || 'unknown'} className="flex items-center justify-between p-3 bg-white dark:bg-white/5 border border-border/60 dark:border-white/5 rounded-xl shadow-sm">
    <div className="flex items-center gap-3">
    <div className="w-8 h-8 rounded-full bg-primary/15 dark:bg-primary text-primary flex items-center justify-center font-bold text-xs">
   ...
   <div className="pt-4 flex gap-3">
   <div className="pt-4 flex gap-3">
   <Button aria-label="Retour à l'étape 3" onClick={() => setStep(3)} variant="ghost" className="w-1/3 py-4 bg-muted text-muted-foreground font-bold rounded-2xl hover:bg-muted transition-colors h-auto">{t('onboarding.actions.back')}</Button>
  ...
  >
   ...
   <span className="text-xs px-2 py-1 bg-primary text-primary-foreground rounded-lg font-bold animate-pulse">
   ...
   {isScanning ? (
   <div className="py-8 flex flex-col items-center justify-center text-center space-y-4 bg-muted/50 dark:bg-white/5 rounded-2xl border border-dashed border-border dark:border-white/10">
   <div className="relative">
    <div className="absolute inset-0 bg-primary rounded-full animate-ping opacity-20"></div>
    <div className="relative w-16 h-16 bg-card rounded-full flex items-center justify-center shadow-xl border-4 border-primary/20 dark:border-primary">
    <Loader2 className="h-8 w-8 text-primary animate-spin" />
    </div>
   </div>
   <div>
    <h3 className="font-bold text-foreground text-lg">{t('onboarding.steps.scanning')}</h3>
   ...
   initialAssets.length === 0 && (
    ...
    className="py-10 bg-muted/50 dark:bg-white/5 rounded-2xl border border-dashed border-border dark:border-white/10 hover:border-primary dark:hover:border-primary cursor-pointer group transition-all text-center"
    >
    <div className="w-12 h-12 rounded-full bg-primary/15 dark:bg-primary text-primary flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
    <Activity className="h-6 w-6" />
    {/* Activity icon used as fallback for BrainCircuit */}
    </div>
    <h3 className="font-bold text-foreground">{t('onboarding.actions.autoScan')}</h3>
    <p className="text-xs text-muted-foreground mt-1">{t('onboarding.actions.autoScanDesc')}</p>
    <div className="mt-4">
    <Button variant="link" size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setManualMode(true); }} aria-label="Ajouter manuellement un actif" className="text-xs text-primary font-bold hover:underline h-auto px-0">{t('onboarding.actions.manualAdd')}</Button>
    </div>
    </div>
   )
   )
   ...
   {initialAssets.map((asset, i) => (
    <div key={`${asset.name || 'unknown'}-${i}`} className="flex items-center justify-between p-3 bg-white dark:bg-white/5 border border-border/60 dark:border-white/5 rounded-xl shadow-sm">
    <div className="flex items-center gap-3">
   ...
   <div className="pt-4 flex gap-3">
   <div className="pt-4 flex gap-3">
   <Button onClick={() => setStep(4)} variant="ghost" aria-label="Retour à l'étape 4" className="w-1/3 py-4 bg-muted text-muted-foreground font-bold rounded-2xl hover:bg-muted transition-colors h-auto">{t('onboarding.actions.back')}</Button>
   <Button onClick={handleStep5} disabled={loading} isLoading={loading} aria-label="Finaliser l'inscription" className="w-2/3 py-4 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-2xl shadow-lg shadow-primary/20 flex items-center justify-center group disabled:bg-muted disabled:text-muted-foreground h-auto">
