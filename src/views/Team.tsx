...
const Team: React.FC = () => {
 ...
 if (dependencies.length > 0) {
  setConfirmData({
  isOpen: true,
  title: t('team.delete.cannotDelete', { defaultValue: 'Suppression impossible' }),
  message: t('team.delete.hasDependenciesDetailed', { defaultValue: 'Cet utilisateur a des éléments liés : {{dependencies}}. Veuillez réassigner ces éléments à un autre membre depuis les modules concernés (Actifs, Risques, Documents) avant de supprimer ce compte.', dependencies: dependencies.join(', ') }),
 ...
 <div className="flex items-center gap-6 relative z-decorator">
  ...
  />
  <circle
  className={`${activityRate >= 80 ? 'text-success-text' : activityRate >= 50 ? 'text-primary' : 'text-warning-text'} transition-all duration-1000 ease-out`}
  strokeWidth="8"
  ...
  <div>
  <h2 className="text-2xl font-black text-foreground mb-1 tracking-tight">{t('team.overview', { defaultValue: 'Vue d\'ensemble' })}</h2>
 ...
 <form onSubmit={(e) => { e.preventDefault(); inviteForm.handleSubmit(handleAddUser)(e); }} className="p-4 sm:p-8 space-y-6">
  ...
  <div className="w-20 h-20 rounded-full bg-muted flex items-center justify-center text-foreground shadow-inner">
  ...
  <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-border/60 dark:border-white/5">
  {inviteSuccess ? (
  <>
  <Button type="button" variant="ghost" onClick={handleCloseInvite}>{t('team.invite.done', { defaultValue: 'Terminé' })}</Button>
  <Button type="button" onClick={() => setInviteSuccess(null)} className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/20">
  ...
  <Button type="submit" disabled={loading || inviteForm.formState.isSubmitting} className="bg-foreground text-background hover:scale-105 transition-transform" isLoading={loading || inviteForm.formState.isSubmitting}>{t('team.invite.send')}</Button>
  </>
  )}
  </div>
 </form>
 ...
 {selectedUser && (
  ...
  <div className="p-4 bg-muted/50 dark:bg-white/5 rounded-2xl mb-4">
  <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">{t('team.edit.account')}</p>
  ...
  <Button type="submit" disabled={loading || editForm.formState.isSubmitting} className="bg-primary text-primary-foreground hover:scale-105 transition-transform" isLoading={loading || editForm.formState.isSubmitting}>{t('team.edit.save')}</Button>
  </div>
  </form>
 )}
 </Drawer>
