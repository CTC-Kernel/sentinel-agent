...
const StatusBar: React.FC<StatusBarProps> = ({ totalNodes, activeLayers, selectedNode, isFullscreen, criticalCount, warningCount, connectionCount }) => {
 ...
 initial={{ y: 40 }}
 animate={{ y: 0 }}
 className="absolute bottom-0 left-0 right-0 h-10 z-voxel-ui flex items-center justify-between px-3 sm:px-4 lg:px-6 bg-card/90 backdrop-blur-2xl border-t border-white/5"
 >
 {/* Left - Stats (compact) */}
...
export const VoxelView: React.FC = () => {
 ...
 >
 <div className="flex items-center justify-between h-12 px-2 sm:px-3 lg:px-4 rounded-xl sm:rounded-2xl bg-card/80 backdrop-blur-2xl border border-white/10 shadow-2xl">
 {/* Left - Logo & Title */}
 ...
 {activeFrameworks?.map(af => (
  <option key={af.frameworkId || 'unknown'} value={af.frameworkCode} className="bg-card">
  {af.frameworkCode}
  </option>
 ))}
 </select>
 ...
 ? 'bg-primary/15 text-primary/50 cursor-wait'
 : 'bg-gradient-to-r from-primary to-violet-600 text-white hover:shadow-lg hover:shadow-primary/30'
 } `}
 >
 ...
 >
 {/* Navigation */}
 <div className="flex flex-col gap-0.5 p-1 rounded-xl bg-card/80 backdrop-blur-xl border border-white/10">
 ...
 <div className="flex flex-col p-1 rounded-xl bg-card/80 backdrop-blur-xl border border-white/10">
 <ToolButton
 ...
 animate={{ opacity: 1, x: 0 }}
 exit={{ opacity: 0, x: 20 }}
 className="absolute top-20 right-16 z-voxel-ui w-44 p-2.5 rounded-xl bg-card/95 backdrop-blur-xl border border-white/10 shadow-2xl"
 >
 <div className="flex items-center justify-between mb-2">
 ...
 <div className="max-h-80 overflow-y-auto p-2">
  ...
  className={`w - full flex items - center gap - 3 px - 3 py - 2.5 rounded - xl text - left transition - all duration - 150 ${index === 0 ? 'bg-primary/15 border border-primary/40' : 'hover:bg-muted/50'
  } `}
  >
