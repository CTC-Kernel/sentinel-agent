...
export const Privacy: React.FC = () => {
 ...
 <div className="h-px w-full md:w-px md:h-16 bg-muted dark:bg-white/10" />
 ...
 <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-6">
  ...
  <span className="text-xs font-medium text-muted-foreground">{t('privacy.stats.toComplete')}</span>
  </div>
  </div>
  <div>
  ...
  <span className="text-xs font-medium text-muted-foreground">{t('privacy.stats.activities')}</span>
  </div>
  </div>
  <div>
 ...
 {
 viewingAssessmentId && (
  <div className="fixed inset-0 z-modal bg-card animate-in fade-in slide-in-from-bottom-10 duration-300 flex flex-col">
  <button
  aria-label="Close"
  onClick={() => {
  ...
  className="absolute top-4 right-4 z-modal p-2 bg-muted rounded-full hover:bg-muted transition-colors"
  >
