...
const AdminDashboard: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'tenants' | 'users' | 'system' | 'audit' | 'links'>('overview');
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-fade-in">
            ...
            <div className="flex space-x-2 border-b border-border/40 pb-4 overflow-x-auto no-scrollbar">
                {(['overview', 'tenants', 'users', 'system', 'audit', 'links'] as const).map((tab) => (
                    <button
                        key={tab || 'unknown'}
            ...
            <div className="min-h-[400px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50">
                ...
                {activeTab === 'links' && <ManagementLinks />}
            </div>
        </div>
    );
};
