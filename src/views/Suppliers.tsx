...
export const Suppliers: React.FC = () => {
 ...
 cell: ({ row }) => (
 <span className={`px-2 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${row.original.status === 'Actif' ? 'bg-success-bg text-success-text' : 'bg-muted text-foreground'
  }`}>
  {row.original.status}
 </span>
 )
 ...
 if (assessmentMode) {
 return (
 <div className="fixed inset-0 z-modal bg-card animate-slide-up">
 ...
 {activeTab === 'suppliers' && (
  ...
  <>
   ...
   className="rounded-2xl shadow-lg shadow-primary/20 font-black uppercase tracking-wider"
