...
export const ExternalAuditPortal: React.FC = () => {
 ...
 <p className="text-muted-foreground font-medium">{t('certifier.portal.loading')}</p>
 </div>
 );
 }
 ...
 <div className="relative z-10">
  ...
  {t('certifier.portal.invitedAs')} <span className="text-muted-foreground font-medium">{auditorEmail}</span>
  </div>
  </div>
  </div>
 </div>
 ...
 {/* Tabs */}
 <div className="flex items-center gap-6 mt-8 border-b border-border/60 dark:border-white/5">
  <button
  onClick={() => setActiveTab('overview')}
  className={`pb-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'overview' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
  >
  {t('certifier.portal.tabs.overview')}
  </button>
  <button
  onClick={() => setActiveTab('findings')}
  className={`pb-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'findings' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
  >
  {t('certifier.portal.tabs.findings')} <span className="bg-muted text-muted-foreground px-1.5 rounded-md text-xs">{auditData.findings.length}</span>
  </button>
  <button
  onClick={() => setActiveTab('controls')}
  className={`pb-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'controls' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
  >
  {t('certifier.portal.tabs.evidence')} <span className="bg-muted text-muted-foreground px-1.5 rounded-md text-xs">{auditData.documents?.length || 0}</span>
  </button>
  {permissions.includes('certify') && (
  <button
  onClick={() => setActiveTab('certify')}
  className={`pb-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'certify' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
  >
 ...
 {activeTab === 'overview' && (
  <div className="bg-card rounded-2xl p-6 border border-border dark:border-white/5 shadow-sm">
 ...
 {activeTab === 'findings' && (
  ...
  <div className="text-center py-12 bg-muted/50 rounded-xl border border-dashed border-border dark:border-white/10">
  <AlertOctagon className="w-8 h-8 text-muted-foreground mx-auto mb-3" />
  <p className="text-muted-foreground font-medium">{t('certifier.portal.findings.empty')}</p>
  </div>
  ) : (
 ...
 {activeTab === 'controls' && (
  <div className="space-y-6">
  <div className="bg-card p-6 rounded-2xl border border-border dark:border-white/5 shadow-sm">
  <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
  ...
  {auditData.documents && auditData.documents.length > 0 ? (
   ...
   className="w-full py-2 flex items-center justify-center gap-2 bg-card border border-border rounded-lg text-sm font-medium text-foreground group-hover:bg-primary/10 dark:group-hover:bg-primary group-hover:text-primary group-hover:border-primary/30 transition-all"
   >
