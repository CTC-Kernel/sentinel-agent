...
export const CertificateUploadSection: React.FC<CertificateUploadProps> = ({ token }) => {
 ...
 <p className="text-muted-foreground">{t('certificate.certificationRecordedDescription', { defaultValue: "Le rapport de certification a été transmis. L'accès à cet audit est désormais archivé." })}</p>
 </div>
 );
 }
 ...
 <label className={`flex flex-col items-center justify-center border-2 border-dashed ${file ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-border'} rounded-lg p-8 cursor-pointer hover:bg-muted transition-colors`}>
  ...
  <span className="text-sm font-medium text-muted-foreground">{t('certificate.dropFile', { defaultValue: 'Déposer le rapport ou le certificat (PDF)' })}</span>
