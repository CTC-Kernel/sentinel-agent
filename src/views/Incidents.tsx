...
import { Plus, Download, LayoutDashboard, List as ListIcon, MoreVertical, BrainCircuit, Siren, Link, AlertTriangle } from '../components/ui/Icons';
...
export const Incidents: React.FC = () => {
    ...
    // Smart Insights
    const incidentInsights = useMemo<SmartInsight[]>(() => {
        if (loading) return [];

        const activeCount = incidents.filter(i => i.status !== 'Fermé' && i.status !== 'Résolu').length;
        const criticalCount = incidents.filter(i => i.severity === Criticality.CRITICAL).length;

        // Most common category
        const catCounts = incidents.reduce((acc, i) => {
            const cat = i.category || 'N/A';
            acc[cat] = (acc[cat] || 0) + 1;
            return acc;
        ...
        return [
            {
                label: t('incidents.stats.total', { defaultValue: 'Total Incidents' }),
                value: incidents.length,
                icon: <Link className="w-5 h-5" />,
                variant: 'primary'
            },
            {
                label: t('incidents.stats.critical', { defaultValue: 'Alertes Critiques' }),
                value: criticalCount,
                subValue: t('incidents.stats.immediateAction', { defaultValue: 'Action immédiate requise' }),
                icon: <AlertTriangle className="w-5 h-5" />,
                variant: 'destructive'
            },
            {
                label: t('incidents.stats.active', { defaultValue: 'Incidents Actifs' }),
                value: activeCount,
                subValue: t('incidents.stats.ongoing', { defaultValue: 'En cours de résolution' }),
                icon: <Siren className="w-5 h-5" />,
                variant: 'warning'
            },
            {
                label: t('incidents.stats.mix', { defaultValue: 'Type Dominant' }),
                value: topCat,
                subValue: t('incidents.stats.vector', { defaultValue: 'Vecteur d\'attaque principal' }),
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'overview' ? (
                    <motion.div
                        ...
                        className="space-y-6"
                    >
                        <SmartSummary insights={incidentInsights} loading={loading} />
                        <IncidentOverview
                            incidents={sortedIncidents} agents={agents} loading={loading} />
                    </motion.div>
                ) : (
                    <motion.div
                        key="incidents"
