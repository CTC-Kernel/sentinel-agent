 ...
 {activeTab === 'documents' && (
  ...
  <div className="mb-6">
   ...
   <Menu as="div" className="relative inline-block text-left">
    ...
    <Menu.Item>
     {({ active }) => (
     <button aria-label={t('documents.newDocument')} onClick={handleCreateClick} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm md:hidden`}>
     <Plus className={`mr-2 h-4 w-4 ${active ? 'text-white' : 'text-primary'}`} /> {t('documents.newDocument')}
     </button>
     )}
    </Menu.Item>
    <Menu.Item>
...
const MemoizedDocumentCard = React.memo(({ doc, viewMode, onSelect, users }: { doc: Document; viewMode: string; onSelect: (d: Document) => void; users: UserProfile[] }) => {
 ...
 className={`glass-premium p-5 rounded-3xl border border-border/40 hover:border-primary/60 hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden flex flex-col gap-4 shadow-sm hover:shadow-apple-sm ${viewMode === 'list' ? 'flex-row items-center border-l-4' : ''}`}
 >
 <div className="flex-1">
 <h4 className="text-base font-black text-foreground truncate tracking-tight">{doc.title}</h4>
 <div className="flex items-center gap-2 mt-2">
