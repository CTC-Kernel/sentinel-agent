...
export const Projects: React.FC = () => {
 ...
 <PremiumPageControl
  ...
  <Transition as={React.Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
   ...
   <Menu.Item>
   {({ active }) => (
    <button aria-label={t('projects.createFromTemplate')} onClick={handleOpenTemplateModal} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
   ...
   {({ active }) => (
    ...
    <Upload className={`mr-2 h-4 w-4 ${active ? 'text-white' : 'text-primary'}`} /> {t('common.importCsv')}
    </button>
   )}
   </Menu.Item>
   <Menu.Item>
   {({ active }) => (
    <button aria-label={t('projects.exportCsv')} onClick={handleExportCSV} className={`${active ? 'bg-accent text-accent-foreground' : 'text-foreground hover:bg-muted'} group flex w-full items-center rounded-lg px-2 py-2 text-sm`}>
 ...
 >
  ...
  <h4 className="flex justify-between px-1 mb-4 text-sm font-bold tracking-wider uppercase text-muted-foreground">
   {statusLabel}
   <span className="bg-muted px-2 py-0.5 rounded-lg text-xs shadow-sm border border-border dark:border-white/5">
  ...
  <div className="flex-1 pr-2 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700">
   ...
   <div className="flex items-center justify-center h-32 text-xs font-medium border-2 border-dashed border-border dark:border-white/5 rounded-xl text-muted-foreground">
