    ...
    ui.horizontal_wrapped(|ui: &mut egui::Ui| {
        ...
        while !current.is_empty() {
            if let Some(start) = current.find("**") {
                // Text before bold
                if start > 0 {
                    ui.label(egui::RichText::new(&current[..start]).font(theme::font_body()).color(theme::text_primary()));
                }
                ...
                if let Some(end) = remaining.find("**") {
                    ...
                    // Unclosed bold
                    ui.label(egui::RichText::new("**").font(theme::font_body()).color(theme::text_primary()));
                    current = remaining;
                }
                ...
                // Plain text
                ui.label(egui::RichText::new(current).font(theme::font_body()).color(theme::text_primary()));
                break;
            }
        }
    });
