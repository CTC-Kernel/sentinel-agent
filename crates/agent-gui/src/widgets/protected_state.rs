...
pub fn protected_state(ui: &mut Ui, icon: &str, title: &str, subtitle: &str) {
    ui.vertical_centered(|ui| {
        ...
        let (rect, _resp) = ui.allocate_exact_size(Vec2::splat(icon_size * 2.5), egui::Sense::hover());
        let center = rect.center();

        // 1. Outer Ring (fading, expanding)
        // Uses a separate time offset for a "ripple" effect
        let ripple_pulse = ((time * pulse_speed * 0.8).rem_euclid(std::f64::consts::PI * 2.0)).sin().max(0.0);
        let max_radius = icon_size * 1.1;
        let ring_radius = (icon_size * 0.6) + (max_radius - (icon_size * 0.6)) * ripple_pulse as f32;
