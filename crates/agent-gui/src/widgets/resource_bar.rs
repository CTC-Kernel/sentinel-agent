...
pub fn resource_bar(ui: &mut Ui, label: &str, value: &str, fraction: f32) {
    ...
    painter.rect_stroke(rect, rounding, egui::Stroke::new(1.0, theme::border()), egui::StrokeKind::Inside);
    ...
    if clamped > 0.0 {
        ...
        let bot_half = Rect::from_min_size(fill_rect.left_center(), Vec2::new(fill_width, bar_height / 2.0));
        
        painter.rect_filled(top_half, CornerRadius { nw: 3, ne: 3, sw: 0, se: 0 }, base_color.linear_multiply(1.3));
        painter.rect_filled(bot_half, CornerRadius { nw: 0, ne: 0, sw: 3, se: 3 }, base_color);
    }
}
